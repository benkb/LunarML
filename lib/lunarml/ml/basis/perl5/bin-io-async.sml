local
    ...
    structure Readable :> sig
              ...
              val fromValue : Perl.value -> readable
              val toValue : readable -> Perl.value
              end = struct
    type vector = Word8Vector.vector
    type readable = Perl.value
        ...
        = DelimCont.withSubCont
              (DelimCont.topLevel, fn cont : (vector list, unit) DelimCont.subcont =>
                                      let val handled = ref false
                                          val readableHandler = Perl.callback (fn _ =>
                                                                                        let fun doRead () : vector list
                                                                                                = let val chunk = Perl.method (stream, "read") #[]
                                                                                                  in if Perl.=== (chunk, Perl.null) then
                                                                                                         nil
                                                                                                     else
                                                                                                         let val chunk : vector = Perl.unsafeFromValue chunk (* Buffer as string *)
                                          ...
                                          val endHandler = Perl.callback (fn _ =>
                                                                                   if not (!handled) then
                                                                                       ( handled := true
                                      ...
                                      in Perl.method (stream, "once") #[Perl.fromWideString "readable", readableHandler]
                                       ; Perl.method (stream, "once") #[Perl.fromWideString "end", endHandler]
                                       ; Perl.method (stream, "read") #[Perl.fromInt 0] (* This seems to be needed to trigger 'readadble'/'end' for interactive stdin *)
                                       ; ()
                                      end
              )
                                      ...
                                      let val handled = ref false
                                          val readableHandler = Perl.callback (fn _ =>
                                                                                        let val chunk : vector
                                                                                                = let val chunk = Perl.method (stream, "read") #[Perl.fromInt n]
                                                                                                  in if Perl.=== (chunk, Perl.null) then
                                                                                                         Word8Vector.fromList []
                                                                                                     else
                                                                                                         Perl.unsafeFromValue chunk (* Buffer as string *)
                                                                                                  end
                                                                                            val () = handled := true
    ...
    fun rawEnded (stream : readable) : bool = Perl.unsafeFromValue (Perl.field (stream, "readableEnded"))
    fun destroy (stream : readable) : unit = (Perl.method (stream, "destroy") #[]; ())
    ...
    fun fromValue (stream : Perl.value) = stream
    ...
    structure Writable :> sig
                  ...
                  val fromValue : Perl.value -> writable
                  val toValue : writable -> Perl.value
              end = struct
    type writable = Perl.value
    type vector = Word8Vector.vector
    fun output (stream, chunk) = let val result = Perl.method (stream, "write") #[Perl.unsafeToValue chunk (* as Uint8Array *)]
                                 in if Perl.isFalsy result then
                                        DelimCont.withSubCont (DelimCont.topLevel, fn cont : (unit, unit) DelimCont.subcont =>
                                                                                      let val onDrain = Perl.callback (fn _ => DelimCont.pushSubCont (cont, fn () => ()))
                                                                                      in Perl.method (stream, "once") #[Perl.fromWideString "drain", onDrain]
                                                                                       ; ()
                                                                                      end
                                                              )
                                    else
    ...
    fun outputAndFlush (stream, chunk) = DelimCont.withSubCont (DelimCont.topLevel, fn cont : (unit, unit) DelimCont.subcont =>
                                                                                       let val callback = Perl.callback (fn _ => DelimCont.pushSubCont (cont, fn () => ()))
                                                                                       in Perl.method (stream, "write") #[Perl.unsafeToValue chunk (* as Uint8Array *), Perl.null, callback]
                                                                                        ; ()
                                                                                       end
                                                               )
    fun closeOut stream = DelimCont.withSubCont (DelimCont.topLevel, fn cont : (unit, unit) DelimCont.subcont =>
                                                                        let val callback = Perl.callback (fn _ => DelimCont.pushSubCont (cont, fn () => ()))
                                                                        in ignore (Perl.method (stream, "end") #[Perl.unsafeToValue callback])
                                                                        end
                                                )
...
fun openIn path = mkInstream (StreamIOImpl.openReadable (Readable.fromValue (Perl.call createReadStream #[Perl.unsafeToValue path (* as Buffer? *)]), path))
fun openOut path = mkOutstream (StreamIOImpl.fromWritable (Writable.fromValue (Perl.call createWriteStream #[Perl.unsafeToValue path (* as Buffer? *)]), IO.BLOCK_BUF, path))
fun openAppend path = let val options = Perl.newObject ()
                          val () = Perl.set (options, Perl.fromWideString "flags", Perl.fromWideString "a")
                      in mkOutstream (StreamIOImpl.fromWritable (Writable.fromValue (Perl.call createWriteStream #[Perl.unsafeToValue path (* as Buffer? *), options]), IO.BLOCK_BUF, path))
                      end
end (* local *)
end (* structure BinIO *)
end; (* local *)

...
fun substring (s : string, i : int, j : int) : string = if i < 0 orelse j < 0 orelse size s < i + j then
                                                            ...
                                                            Perl.unsafeFromValue (Perl.method (Perl.unsafeToValue s, "subarray") #[Perl.fromInt i, Perl.fromInt (i + j)])
fun extract (s : string, i : int, NONE : int option) : string = if i < 0 orelse size s < i then
                                                                    raise Subscript
                                                                else
                                                                    Perl.unsafeFromValue (Perl.method (Perl.unsafeToValue s, "subarray") #[Perl.fromInt i, Perl.fromInt (size s)])

...
fun modTime (path : string) : Time.time = let val options = Perl.newObject ()
                                              val () = Perl.setField (options, "bigint", Perl.fromBool true)
                                              val stat = Perl.call fs.statSync #[Perl.fromWideString (Perl.decodeUtf8 path), options]
                                              val mtimeNs = Perl.unsafeFromValue (Perl.field (stat, "mtimeNs")) : IntInf.int
                                          in Time.fromNanoseconds mtimeNs
                                          end
fun fileSize (path : string) : Position.int = let val stat = Perl.call fs.statSync #[Perl.fromWideString (Perl.decodeUtf8 path)]
                                              in Perl.unsafeFromValue (Perl.field (stat, "size")) : Position.int
                                              end
fun setTime (path : string, t : Time.time option) = let val u = Time.toReal (case t of NONE => Time.now () | SOME u => u)
                                                    in ignore (Perl.call fs.utimesSync #[Perl.fromWideString (Perl.decodeUtf8 path), Perl.fromReal u, Perl.fromReal u])
                                                    end

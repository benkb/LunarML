...
fun sub (s : string, i : int) : char = if i < 0 orelse size s <= i then
                                           ...
                                           Perl.unsafeFromValue (Perl.sub (Perl.unsafeToValue s, Perl.fromInt i)) : char
...
fun map (f : char -> char) (s : string) : string = let val s = Perl.unsafeToValue s
                                                   in Perl.unsafeFromValue (Perl.method (s, "map") #[Perl.unsafeToValue f])
                                                   end
fun x < y = _primCall "String.<" (x, y)

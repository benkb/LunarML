...
struct
  ...
  | BACKEND_LUAJIT
  | BACKEND_PERL of CodeGenPl.code_style
  ...
  fun emit (opts as {backend = BACKEND_LUA runtime, ...}: options)
    ...
    | emit (opts as {backend = BACKEND_PERL style, ...}) _ fileName cont nextId
        cexp export =
        let
          val timer = Timer.startCPUTimer ()
          val contEscapeMap = CpsAnalyze.contEscape (cont, cexp)
          ...
          val mlinit_perl =
            case style of
              CodeGenPl.DIRECT_STYLE =>
                OS.Path.joinDirFile {dir = #libDir opts, file = "mlinit.js"}
            | CodeGenPl.CPS =>
                OS.Path.joinDirFile {dir = #libDir opts, file = "mlinit-cps.js"}
          val mlinit = InitFile.readFile (InitFile.PL, mlinit_perl)
          val perlctx =
            { nextPlId = nextId
            , contEscapeMap = contEscapeMap
            , style = style
          ...
          val perl =
            case (style, export) of
              (CodeGenPl.DIRECT_STYLE, ToFSyntax.NO_EXPORT) =>
                CodeGenPl.doProgramDirect perlctx cont cexp
            | (CodeGenPl.DIRECT_STYLE, ToFSyntax.EXPORT_VALUE) =>
                CodeGenPl.doProgramDirectDefaultExport perlctx cont cexp
            | (CodeGenPl.DIRECT_STYLE, ToFSyntax.EXPORT_NAMED names) =>
                CodeGenPl.doProgramDirectNamedExport perlctx cont cexp names
            | (CodeGenPl.CPS, ToFSyntax.NO_EXPORT) =>
                CodeGenPl.doProgramCPS perlctx cont cexp
            | (CodeGenPl.CPS, ToFSyntax.EXPORT_VALUE) =>
                CodeGenPl.doProgramCPSDefaultExport perlctx cont cexp
            | (CodeGenPl.CPS, ToFSyntax.EXPORT_NAMED names) =>
                CodeGenPl.doProgramCPSNamedExport perlctx cont cexp names
          val codegenTime = Time.toMicroseconds
            (#usr (Timer.checkCPUTimer timer))
          val perl = PlTransform.doProgram {nextVId = nextId} perl
          val codetransTime = Time.toMicroseconds
            (#usr (Timer.checkCPUTimer timer))
          val hasImports = not (List.null (!(#imports perlctx)))
          val imports = PlWriter.doImports (!(#imports perlctx))
          val usedLib = StringSet.toList
            (PlSyntax.predefinedIdsInBlock (perl, StringSet.empty))
          val perl = PlWriter.doProgram perl
          val writeTime = Time.toMicroseconds (#usr (Timer.checkCPUTimer timer))
          val mlinit = InitFile.eliminateUnusedChunks (mlinit, usedLib)
          val outs = TextIO.openOut
            (Option.getOpt (#output opts, base ^ ".mperl")) (* may raise Io *)
          val () = TextIO.output (outs, imports)
          val () =
            if hasImports then () else TextIO.output (outs, "\"use strict\";\n")
          val () = InitFile.output (outs, mlinit)
          val () = TextIO.output (outs, perl)
          val () = TextIO.closeOut outs
          val () =
            if #printTimings opts then
              print
  ...
  fun doCompile (opts: options) fileName
    ...
    let
      val pathMap = List.foldl StringMap.insert' StringMap.empty
        ...
        , ( "TARGET_LANG"
          , case #backend opts of
              BACKEND_LUA _ => "lua" | BACKEND_LUAJIT => "luajit"
            | BACKEND_PERL CodeGenPl.DIRECT_STYLE => "pl"
            | BACKEND_PERL CodeGenPl.CPS => "pl-cps"
        ...
        , ( "DELIMITED_CONTINUATIONS"
          , case #backend opts of
            ...
            | BACKEND_PERL CodeGenPl.CPS => "multishot"
            | _ => "none"
          )
        ]
      ...
      val exportAsRecord =
        case #backend opts of
          BACKEND_LUA _ => true
        | BACKEND_LUAJIT => true
        | BACKEND_PERL CodeGenPl.DIRECT_STYLE => false
        | BACKEND_PERL CodeGenPl.CPS => true
    ...
    | CodeGenLua.CodeGenError message =>
        (print (message ^ "\n"); OS.Process.exit OS.Process.failure)
    | CodeGenPl.CodeGenError message =>
        (print (message ^ "\n"); OS.Process.exit OS.Process.failure)
  ...
  | OPT_TARGET_PERL (* --nodejs *)
  | OPT_TARGET_PERL_CPS (* --nodejs-cps *)
  | OPT_HELP (* -h,--help *)
  ...
  fun parseArgs (opts: options) args =
    ...
    | SOME (OPT_TARGET_PERL, args) =>
        parseArgs (S.set.backend (BACKEND_PERL CodeGenPl.DIRECT_STYLE) opts)
          args
    | SOME (OPT_TARGET_PERL_CPS, args) =>
        parseArgs (S.set.backend (BACKEND_PERL CodeGenPl.CPS) opts) args

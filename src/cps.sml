  ...
  *)
  ...
  in
    fun isDiscardable (PrimOp {primOp = F.IntConstOp _, ...}) = true
      ...
      | isDiscardable (PrimOp {primOp = F.PlCallOp, ...}) = false
      | isDiscardable (PrimOp {primOp = F.PlMethodOp, ...}) = false
      | isDiscardable (PrimOp {primOp = F.PlNewOp, ...}) = false
    ...
    and freeVarsInExp (bound, Let {decs, cont}, acc) =
      ...
      | freeVarsInExp (bound, App {applied, cont = _, args}, acc) =
...
struct
  ...
  in
          ...
          let
            val s =
              List.foldl (fn ({body, ...}, acc) => goExp (g, body, acc)) acc
                defs
          in
            List.app (fn {name, ...} => TypedSyntax.VIdTable.insert g (name, s))
              defs;
            acc
          end
    ...
    and goExp (g, C.Let {decs, cont}, acc) =
      ...
      | goExp (_, C.App {applied, cont = _, args}, acc) =
  ...
  in
          ...
          (case TypedSyntax.VIdTable.find env v of
             SOME r =>
               ...
               in
                 ...
                 , ref_write = ref_write
                 , other = other
                 , returnConts = C.CVarSet.add (returnConts, cont)
                 , labels = labels
                 }
               end
           | NONE => ())
    ...
    in
      fun goSimpleExp
        ...
        | goSimpleExp (env, _, _, _, _, C.PrimOp {primOp = _, tyargs = _, args}) =
      ...
      and goDec (env, renv, cenv, crenv) =
        fn C.ValDec {exp, results} =>
          ...
          ; List.app (fn SOME result => add (env, result) | NONE => ()) results
          )
         | C.RecDec defs =>
          let
            val recursiveEnv =
          ...
          in
            TypedSyntax.VIdMap.appi
              (fn (f, v) => TypedSyntax.VIdTable.insert env (f, v)) recursiveEnv;
            List.app
         ...
         | C.RecContDec defs =>
          let
            val recursiveCEnv =
              List.foldl
                (fn ((f, _, _), m) => C.CVarMap.insert (m, f, ref neverUsedCont))
                C.CVarMap.empty defs
          in
...
struct
  ...
  in
    ...
    and substCExp
      ...
      | substCExp (subst, csubst, C.App {applied, cont, args}) =
          C.App
    ...
    and isDiscardableExp
          ...
          (case ListUtil.foldlOption isDiscardableDec env decs of
           ...
           | NONE => false)
      | isDiscardableExp
          (env, C.App {applied = C.Var applied, cont = _, args = _}) =
      ...
      case dec of
        C.ValDec
          ...
          in
            ...
            , C.App {applied = f, cont = name, args = [arg]}
            )
          end
      | C.ValDec
          { exp =
...
struct
  ...
  in
    ...
    and go (table, level, C.Let {decs, cont}, acc) =
      ...
      | go (table, level, C.App {applied = _, cont, args = _}, acc) =

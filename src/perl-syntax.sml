...
sig
  ...
  val asciiStringAsWide: string -> PlConst
  datatype BinaryOp =
    PLUS
  | MINUS
  | TIMES
  ...
  | SwitchStat of
      Exp
      * (PlConst * Stat vector) list (* switch (e) { case c0: { ... } case c1: { ... } } *)
...
struct
  ...
  | SwitchStat of
      Exp
      * (PlConst * Block) list (* switch (e) { case c0: { ... } case c1: { ... } } *)
  | BreakStat of Id option
  | ContinueStat of Id option
  | DefaultExportStat of Exp (* export default <expression> *)
  | NamedExportStat of
...
structure PlWriter :>
sig
  val doProgram: PlSyntax.Block -> string
  val doImports: {specs: (string * PlSyntax.Id) list, moduleName: string} list
                 -> string
end =
struct
  fun smlNameToPlChar #"_" = "_"
    | smlNameToPlChar #"'" = "$PRIME"
    | smlNameToPlChar #"!" = "$EXCLAM"
    | smlNameToPlChar #"%" = "$PERCENT"
    | smlNameToPlChar #"&" = "$AMPER"
    | smlNameToPlChar #"$" = "$DOLLAR"
    | smlNameToPlChar #"#" = "$HASH"
    | smlNameToPlChar #"+" = "$PLUS"
    | smlNameToPlChar #"-" = "$MINUS"
    | smlNameToPlChar #"/" = "$SLASH"
    | smlNameToPlChar #":" = "$COLON"
    | smlNameToPlChar #"<" = "$LT"
    | smlNameToPlChar #"=" = "$EQ"
    | smlNameToPlChar #">" = "$GT"
    | smlNameToPlChar #"?" = "$QUESTION"
    | smlNameToPlChar #"@" = "$AT"
    | smlNameToPlChar #"\\" = "$BACKSLASH"
    | smlNameToPlChar #"~" = "$TILDE"
    | smlNameToPlChar #"`" = "$GRAVE"
    | smlNameToPlChar #"^" = "$HAT"
    | smlNameToPlChar #"|" = "$BAR"
    | smlNameToPlChar #"*" = "$ASTER"
    | smlNameToPlChar x =
        if Char.isAlphaNum x then String.str x
        else raise Fail "smlNameToPl: invalid character"
  fun smlNameToPl name = String.translate smlNameToPlChar name
  val PlReservedWords = StringSet.fromList
    [ "await"
    , "break"
    , "case"
    , "catch"
  ...
  fun isIdentifier name =
    isIdentifierName name andalso not (StringSet.member (PlReservedWords, name))
  fun idToPl (PlSyntax.PredefinedId name) = name
    | idToPl (PlSyntax.UserDefinedId (TypedSyntax.MkVId (name, n))) =
        smlNameToPl name ^ "$"
    ...
    fun doChar 0x5C = "\\\\"
      ...
      | doChar i =
          if i < 0 orelse 0x10000 <= i then
            raise Fail "this string cannot be expressed in Perl"
  ...
  fun doKey (S.IntKey n) =
        toWideStringLit (PlSyntax.asciiStringAsIntVector (intToString n))
    | doKey (S.StringKey s) =
        if isIdentifier s then s
        else toWideStringLit (PlSyntax.asciiStringAsIntVector s)
  ...
  fun doExp (_, S.ConstExp ct) : Fragment list -> Fragment list = doConst ct
    ...
    | doExp (_, S.VarExp id) =
        (fn rest => Fragment (idToPl id) :: rest)
    ...
    | doExp (_, S.FunctionExp (parameters, body)) =
        (fn rest =>
           ...
           ::
           commaSepV
             (Vector.map (fn id => fn rest => Fragment (idToPl id) :: rest)
                parameters)
  ...
  and doStat (S.LetStat variables) =
        (fn rest =>
           ...
           commaSepV
             (Vector.map
                (fn (id, NONE) =>
                   (fn rest => Fragment (idToPl (S.UserDefinedId id)) :: rest)
                  | (id, SOME init) =>
                   (fn rest =>
                      Fragment (idToPl (S.UserDefinedId id)) :: Fragment " = "
        ...
        (fn rest =>
           ...
           doBlock body
             (DecreaseIndent :: Indent :: Fragment "} catch ("
              :: Fragment (idToPl (S.UserDefinedId exnName)) :: Fragment ") {"
              :: IncreaseIndent :: LineTerminator
              ::
              doBlock catch
                (DecreaseIndent :: Indent :: Fragment "}" :: LineTerminator
    ...
    | doStat (S.BlockStat (SOME label, block)) =
        (fn rest =>
           Indent :: Fragment (idToPl label) :: Fragment ": {" :: IncreaseIndent
           :: LineTerminator
           ::
           doBlock block
             (DecreaseIndent :: Indent :: Fragment "}" :: LineTerminator :: rest))
    ...
    | doStat (S.LoopStat (SOME label, block)) =
        (fn rest =>
           Indent :: Fragment (idToPl label) :: Fragment ": for (;;) {"
           :: IncreaseIndent :: LineTerminator
           ::
           doBlock block
             (DecreaseIndent :: Indent :: Fragment "}" :: LineTerminator :: rest))
        ...
        (fn rest =>
           ...
           commaSepV
             (Vector.map
                (fn (v, name) =>
                   fn rest =>
                     Fragment (idToPl v) :: Fragment " as " :: Fragment name
                     :: rest) entities)
             (Fragment "};" :: LineTerminator :: rest))
  and doBlock stats =
  ...
  (*: val doImports : { specs : (string * PlSyntax.Id) list, moduleName : string } list -> string *)
        ...
        in
          case (default, named) of
            ([], []) =>
              ...
              :: Fragment ";" :: LineTerminator :: rest
          | ((_, defaultId) :: _, []) =>
              Fragment "import " :: Fragment (idToPl defaultId)
                ...
                (List.map
                   (fn (name, vid) =>
                      fn rest =>
                        ...
                        :: Fragment (idToPl vid) :: rest) named)
                (Fragment "} from " :: Fragment (toStringLit moduleName)
                 :: Fragment ";" :: LineTerminator :: rest)
          | ((_, defaultId) :: _, _ :: _) =>
              Fragment "import " :: Fragment (idToPl defaultId)

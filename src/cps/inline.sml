          ...
          (case (primOp, args) of
           ...
           | (F.PrimCall P.JavaScript_new, [ctor, C.Var args]) =>
               (case TypedSyntax.VIdMap.find (env, args) of
                ...
                | _ => NOT_SIMPLIFIED)
           | (F.PrimCall P.Perl_call, [f, C.Var args]) =>
               (case TypedSyntax.VIdMap.find (env, args) of
                  SOME
                    { exp =
                        SOME (C.PrimOp {primOp = F.VectorOp, tyargs = _, args})
                    ...
                    SIMPLE_EXP
                      (C.PrimOp
                         {primOp = F.PlCallOp, tyargs = [], args = f :: args})
                | _ => NOT_SIMPLIFIED)
           | (F.PrimCall P.Perl_method, [obj, name, C.Var args]) =>
               (case TypedSyntax.VIdMap.find (env, args) of
                  SOME
                    { exp =
                        SOME (C.PrimOp {primOp = F.VectorOp, tyargs = _, args})
                ...
                | _ => NOT_SIMPLIFIED)
           | (F.PrimCall P.Perl_new, [ctor, C.Var args]) =>
               (case TypedSyntax.VIdMap.find (env, args) of
                  SOME
                    { exp =
                        SOME (C.PrimOp {primOp = F.VectorOp, tyargs = _, args})
                    ...
                    SIMPLE_EXP
                      (C.PrimOp
                         {primOp = F.PlNewOp, tyargs = [], args = ctor :: args})
                | _ => NOT_SIMPLIFIED)

...
struct
  ...
  in
    ...
    and simplifyDec (ctx: Context, appliedCont: C.CVar option)
            ...
            val results =
              List.map
                (fn result as SOME name =>
                   if CpsDeadCodeAnalysis.isUsed (#dead_code_analysis ctx, name) then
                     result
                   else
                     NONE
                  | NONE => NONE) results
                             ...
                             case shouldTransformParams of
                               SOME paramTransforms =>
                                 let
                                   ...
                                   val result' =
                                     CpsSimplify.renewVId (#base ctx, result)
                                   val wrapperBody =
                                     let
                                       val k =
                                         CpsSimplify.genContSym (#base ctx)
                                       val params' =
                                         List.map
                                           (fn p =>
                                   ...
                                   val env = TypedSyntax.VIdMap.insert
                                     ( env
                                     , result
                                     , { exp = SOME wrapperBody
                                       , isDiscardableFunction =
                                           CpsSimplify.isDiscardableExp
                                             (env, body)
                                       }
                                     )
                                   val dec = C.ValDec
      ...
      | C.RecDec defs =>
          ...
          then
            let
              fun transform ({name, contParam, params, body, attr}, (env, acc)) =
                let
                  val shouldTransformParams =
                    ...
                    SOME paramTransforms =>
                      let
                        ...
                        val env = TypedSyntax.VIdMap.insert
                          ( env
                          , name
                          , {exp = SOME wrapper, isDiscardableFunction = false}
                          )
                      in
                        ( env
                        , { name = name'
              ...
              fun tryConvertToLoop
                ...
                then
                  let
                    ...
                    val body' =
                      C.recurseCExp
                        (fn call as C.App {applied = C.Var applied, cont, args} =>
          ...
          (case CpsUsageAnalysis.getContUsage (#cont_usage ctx, name) of
           ...
           | {direct = direct_usage, indirect = indirect_usage} =>
               ...
               then (* Inline small continuations *)
                 ...
                 let
                   val shouldTransformParams =
                     if indirect_usage = NEVER then
                       let
                         val t =
                           List.map (tryUnpackContParam (ctx, #usage ctx))
                             params
                       in
                         if
                           List.exists
      ...
      | C.RecContDec defs =>
          ...
          then
            ...
            let
              ...
              val cenv =
                List.foldl
                  (fn ({origName, inline, ...}, cenv) =>
                     C.CVarMap.insert (cenv, origName, inline)) cenv defs'
              val dec = C.RecContDec
                (List.map
                   (fn {newName, newParams, env, body, ...} =>
                      ( newName
                      , newParams
                ...
                (case TypedSyntax.VIdMap.find (env, applied) of
                   SOME
                     ...
                     let
                       ...
                       val canOmitAlphaConversion =
                         case
                           CpsUsageAnalysis.getValueUsage (#usage ctx, applied)
                         of
                           { call = ONCE
                           , project = NEVER
                           , ref_read = NEVER
                 ...
                 | SOME {exp = _, isDiscardableFunction = true} =>
                     (case C.CVarMap.find (cenv, cont) of
                        SOME (params, _) =>
                          if not (List.exists Option.isSome params) then
                            ( #simplificationOccurred (#base ctx) := true
                            ; C.AppCont
                                { applied = cont
                                , args =
                                    List.map (fn _ => C.Unit (* dummy *)) params
                                }
                            )
                          else
                            C.App
                      ...
                      | _ =>
                          C.App
                            {applied = C.Var applied, cont = cont, args = args})
                 | _ =>

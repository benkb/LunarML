...
structure DesugarPatternMatches :>
sig
  ...
  val desugarPatternMatches:
    Context
    -> { doExp: FSyntax.Exp -> FSyntax.Exp
       , doDec: FSyntax.Dec -> FSyntax.Dec
       , doDecs: FSyntax.Dec list -> FSyntax.Dec list
       }
end =
struct
  exception DesugarError of SourcePos.span list * string
  ...
  fun isWildcardPat (F.WildcardPat _) = true
    ...
    | isWildcardPat
        (F.RecordPat {sourceSpan = _, fields, ellipsis = NONE, allFields = _}) =
        List.all (fn (_, pat) => isWildcardPat pat) fields
    | isWildcardPat
        (F.RecordPat
           {sourceSpan = _, fields, ellipsis = SOME basePat, allFields = _}) =
        isWildcardPat basePat
        andalso List.all (fn (_, pat) => isWildcardPat pat) fields
  ...
  fun desugarPatternMatches (ctx: Context) :
    { doExp: F.Exp -> F.Exp
    , doDec: F.Dec -> F.Dec
    , doDecs: F.Dec list -> F.Dec list
    } =
    let
      fun doExp exp0 =
        (case exp0 of
           F.PrimExp (primOp, tyargs, args) =>
             F.PrimExp (primOp, tyargs, List.map doExp args)
         | F.VarExp _ => exp0
         | F.RecordExp fields =>
             F.RecordExp (List.map (fn (label, e) => (label, doExp e)) fields)
         ...
         | F.PackExp {payloadTy, exp, packageTy} =>
             F.PackExp
               {payloadTy = payloadTy, exp = doExp exp, packageTy = packageTy}
         | F.CaseExp
             { sourceSpan = _
             , subjectExp
             , subjectTy = _
             , matches = [(F.VarPat (_, vid, ty'), exp2 as F.VarExp vid')]
             , matchType = _
             , resultTy = _
             } =>
             if TypedSyntax.eqVId (vid, vid') then doExp subjectExp
             else F.LetExp ([F.ValDec (vid, SOME ty', doExp subjectExp)], exp2)
         | F.CaseExp
             ...
             let
               val canIgnore =
                 case matches of
                   [(pat, innerExp)] =>
                     if isWildcardPat pat then SOME innerExp else NONE
                 | _ => NONE
             in
               case canIgnore of
                 SOME innerExp =>
               ...
               | NONE =>
                           ...
                           (case matchType of
                              TypedSyntax.CASE =>
                                F.RaiseExp
                                  ( sourceSpan
                                  , resultTy
                                  , F.VarExp InitialEnv.VId_Match
                                  )
                            | TypedSyntax.VAL =>
                                F.RaiseExp
                                  ( sourceSpan
                                  , resultTy
                                  , F.VarExp InitialEnv.VId_Bind
                                  )
                            | TypedSyntax.HANDLE =>
                                F.RaiseExp
                                  (SourcePos.nullSpan, resultTy, examinedExp))
                       | go ((pat, innerExp) :: rest) =
                           let
                             val binders = genBinders examinedExp subjectTy pat
                             val matcher = genMatcher examinedExp subjectTy pat
                           in
                             if isExhaustive pat then
                               List.foldr
                                 (fn (valbind, exp) =>
                                    F.LetExp ([F.ValDec valbind], exp))
                                 (doExp innerExp) binders
                             else
                               F.IfThenElseExp
                                 ( matcher
                                 , List.foldr
                   ...
                   in
                     F.LetExp
                       ( [F.ValDec
                            (examinedVId, SOME subjectTy, doExp subjectExp)]
                       , go matches
                       )
                   end
             end
         ...
         | F.ExportModule entities =>
             F.ExportModule
               (Vector.map (fn (name, exp) => (name, doExp exp)) entities))
      and doDec (F.ValDec (vid, optTy, exp)) =
            F.ValDec (vid, optTy, doExp exp)
        | doDec (F.RecValDec valbinds) =
            F.RecValDec
              (List.map (fn (v, ty, exp) => (v, ty, doExp exp)) valbinds)
        | doDec (F.UnpackDec (tv, kind, vid, ty, exp)) =
            F.UnpackDec (tv, kind, vid, ty, doExp exp)
        | doDec (F.IgnoreDec exp) =
            F.IgnoreDec (doExp exp)
        | doDec (dec as F.DatatypeDec _) = dec
        | doDec (dec as F.ExceptionDec _) = dec
        | doDec (dec as F.ESImportDec _) = dec
      and genMatcher _ _ (F.WildcardPat _) : F.Exp =
        ...
        | genMatcher exp (recordTy as F.RecordType fieldTypes)
            (F.RecordPat {sourceSpan, fields, ellipsis = NONE, allFields = _}) =
            List.foldr
              (fn ((label, pat), e) =>
                 case Syntax.LabelMap.find (fieldTypes, label) of
                   SOME fieldTy =>
                     ...
                     in
                       F.SimplifyingAndalsoExp (exp, e)
                     end
                 | NONE =>
                     raise DesugarError
                       ( [sourceSpan]
                       , "internal error: record field not found (fieldTypes="
                         ^ FSyntax.PrettyPrint.print_Ty recordTy ^ ", "
                         ^ Syntax.PrettyPrint.print_Label label ^ ")"
                       )) (F.VarExp InitialEnv.VId_true) fields
        | genMatcher exp (recordTy as F.RecordType fieldTypes)
            (F.RecordPat
               {sourceSpan, fields, ellipsis = SOME basePat, allFields = _}) =
            let
              val restTypes =
                List.foldl
                  (fn ((label, _), fieldTypes) =>
                     #1 (Syntax.LabelMap.remove (fieldTypes, label))) fieldTypes
                  fields
              val restExp = F.RecordExp
                (Syntax.LabelMap.foldri
                   (fn (label, _, xs) =>
                      ( label
                      , F.ProjectionExp
                          {label = label, record = exp, fieldTypes = fieldTypes}
                      ) :: xs) [] restTypes)
        ...
        | genMatcher _ _
            (F.RecordPat {sourceSpan, fields = _, ellipsis = _, allFields = _}) =
            raise DesugarError
              ( [sourceSpan]
              , "internal error: record pattern against non-record type"
              )
        | genMatcher exp ty
            (F.ValConPat
               {sourceSpan, info, payload = SOME (payloadTy, payloadPat)}) =
            (case info of
                 ...
                 let
                   val elemTy =
                     ...
                     | _ =>
                         raise DesugarError
                           ( [sourceSpan]
                           , "internal error: nil pattern with invalid type"
                           )
                   val hdExp =
                     F.PrimExp
                       (F.PrimCall Primitives.List_unsafeHead, [elemTy], [exp])
                   val tlExp =
                     F.PrimExp
                       (F.PrimCall Primitives.List_unsafeTail, [elemTy], [exp])
                   val payload =
                     genMatcher (F.TupleExp [hdExp, tlExp]) payloadTy payloadPat
                 in
                   F.SimplifyingAndalsoExp
                     ( F.PrimExp
                         ( F.PrimCall Primitives.Bool_not
                         , []
                         , [F.PrimExp
                              (F.PrimCall Primitives.List_null, [elemTy], [exp])]
                         )
                     , payload
                     )
                 end
             | {representation = Syntax.REP_REF, tag = "ref", ...} =>
                 genMatcher
                   (F.PrimExp
                      (F.PrimCall Primitives.Ref_read, [payloadTy], [exp]))
        ...
        | genMatcher exp ty (F.ValConPat {sourceSpan, info, payload = NONE}) =
            (case info of
             ...
             | {representation = Syntax.REP_BOOL, tag = "false", ...} =>
                 F.PrimExp (F.PrimCall Primitives.Bool_not, [], [exp])
             | {representation = Syntax.REP_LIST, tag = "nil", ...} =>
                 ...
                 in
                   F.PrimExp (F.PrimCall Primitives.List_null, [elemTy], [exp])
                 end
        ...
        | genMatcher exp _ (F.VectorPat (_, pats, ellipsis, elemTy)) =
            let
              val vectorLengthExp = F.PrimExp
                ( F.PrimCall (Primitives.Vector_length Primitives.INT)
                , [elemTy]
                , [exp]
                )
              ...
              val e0 =
                if ellipsis then
                  F.PrimExp
                    ( F.PrimCall (Primitives.Int_GE Primitives.INT)
                    , []
                    , [vectorLengthExp, expectedLengthExp]
                    )
                else
                  F.PrimExp
                   ...
                   let
                     val exp =
                       genMatcher
                         (F.PrimExp
                            ( F.PrimCall
                                (Primitives.Unsafe_Vector_sub Primitives.INT)
                            , [elemTy]
      ...
      and genBinders _ _ (F.WildcardPat _) = []
        ...
        | genBinders exp _ (F.VarPat (_, vid, ty)) =
            [(vid, SOME ty, exp)]
        | genBinders exp (F.RecordType fieldTypes)
            (F.RecordPat
               {sourceSpan = _, fields, ellipsis = NONE, allFields = _}) =
            List.concat
              (List.map
                 (fn (label, innerPat) =>
                    genBinders
                      (F.ProjectionExp
                         {label = label, record = exp, fieldTypes = fieldTypes})
                      (Syntax.LabelMap.lookup (fieldTypes, label)) innerPat)
                 fields)
        | genBinders exp (F.RecordType fieldTypes)
            (F.RecordPat
               {sourceSpan = _, fields, ellipsis = SOME basePat, allFields = _}) =
            ...
            in
              genBinders restExp (F.RecordType restTypes) basePat
              @
              List.concat
                (List.map
                   (fn (label, innerPat) =>
                      genBinders
                        (F.ProjectionExp
                           ...
                           , record = exp
                           , fieldTypes = fieldTypes
                           }) (Syntax.LabelMap.lookup (fieldTypes, label))
                        innerPat) fields)
            end
        | genBinders _ _
            (F.RecordPat {sourceSpan, fields = _, ellipsis = _, allFields = _}) =
                 ...
                 let
                   ...
                   val tlExp =
                     F.PrimExp
                       (F.PrimCall Primitives.List_unsafeTail, [elemTy], [exp])
                 in
                   genBinders (F.TupleExp [hdExp, tlExp]) payloadTy payloadPat
                 end
             | _ =>
                 genBinders
                   (F.PrimExp (F.DataPayloadOp info, [payloadTy], [exp]))
      ...
      and isExhaustive (F.WildcardPat _) = true
        ...
        | isExhaustive
            (F.RecordPat
               {sourceSpan = _, fields, ellipsis = NONE, allFields = _}) =
            List.all (fn (_, e) => isExhaustive e) fields
        | isExhaustive
            (F.RecordPat
               {sourceSpan = _, fields, ellipsis = SOME basePat, allFields = _}) =
            isExhaustive basePat
            andalso List.all (fn (_, e) => isExhaustive e) fields
        | isExhaustive (F.ValConPat _) = false (* TODO *)
        | isExhaustive (F.ExnConPat _) = false
        | isExhaustive (F.LayeredPat (_, _, _, innerPat)) =
...
structure DecomposeValRec :>
sig
  val doExp: FSyntax.Exp -> FSyntax.Exp
end =
struct
  structure F = FSyntax
  fun doExp (F.PrimExp (primOp, tyargs, args)) =
        F.PrimExp (primOp, tyargs, List.map doExp args)
    | doExp (exp as F.VarExp _) = exp
    | doExp (F.RecordExp fields) =
        F.RecordExp (List.map (fn (label, exp) => (label, doExp exp)) fields)
    | doExp (F.LetExp (decs, exp)) =
        F.LetExp (List.concat (List.map doDec decs), doExp exp)
    | doExp (F.AppExp (exp1, exp2)) =
        F.AppExp (doExp exp1, doExp exp2)
    ...
    | doExp
        ...
        F.CaseExp
          ...
          , subjectTy = subjectTy
          , matches = List.map (fn (pat, exp) => (pat, doExp exp)) matches
          , matchType = matchType
          , resultTy = resultTy
          }
    | doExp (F.FnExp (vid, ty, exp)) =
        F.FnExp (vid, ty, doExp exp)
    | doExp (F.ProjectionExp {label, record, fieldTypes}) =
        F.ProjectionExp
          {label = label, record = doExp record, fieldTypes = fieldTypes}
    | doExp (F.TyAbsExp (tv, kind, exp)) =
        F.TyAbsExp (tv, kind, doExp exp)
    | doExp (F.TyAppExp (exp, ty)) =
        F.TyAppExp (doExp exp, ty)
    | doExp (F.PackExp {payloadTy, exp, packageTy}) =
        F.PackExp
          {payloadTy = payloadTy, exp = doExp exp, packageTy = packageTy}
    | doExp (e as F.BogusExp _) = e
    | doExp (e as F.ExitProgram) = e
    | doExp (F.ExportValue exp) =
        F.ExportValue (doExp exp)
    | doExp (F.ExportModule entities) =
        F.ExportModule
          (Vector.map (fn (name, exp) => (name, doExp exp)) entities)
  and doDec (F.ValDec (vid, optTy, exp)) =
        [F.ValDec (vid, optTy, doExp exp)]
    | doDec (F.RecValDec valbinds) =
        let
          val bound =
            List.foldl
              (fn ((vid, _, _), set) => TypedSyntax.VIdSet.add (set, vid))
              TypedSyntax.VIdSet.empty valbinds
          val map =
            List.foldl
              (fn (def as (vid, _, exp), map) =>
                 TypedSyntax.VIdMap.insert
                   ( map
                   , vid
                   , { def = def
                     , dests = TypedSyntax.VIdSet.intersection
                         ( F.freeVarsInExp (TypedSyntax.VIdSet.empty, exp)
                 ...
                 val dec =
                   ...
                   | scc =>
                       F.RecValDec
                         (List.map
                            (fn vid =>
                               #def (TypedSyntax.VIdMap.lookup (map, vid))) scc)
               in
                 dec :: decs
               end) [] sccs
        end
...
structure DeadCodeElimination :>
sig
  val doExp: FSyntax.Exp
             -> TypedSyntax.VIdSet.set
             -> TypedSyntax.VIdSet.set * FSyntax.Exp
end =
struct
  structure F = FSyntax
  fun isDiscardablePrimOp (F.IntConstOp _) = true
    ...
    | isDiscardablePrimOp F.PlCallOp = false
    | isDiscardablePrimOp F.PlMethodOp = false
    | isDiscardablePrimOp F.PlNewOp = false
  ...
  fun isDiscardable (F.PrimExp (primOp, _, args)) =
    ...
    | isDiscardable (F.RecordExp fields) =
    ...
    | isDiscardable (F.PackExp {payloadTy = _, exp, packageTy = _}) =
        isDiscardable exp
  ...
  fun doPat (F.WildcardPat _) acc = acc
    | doPat (F.SConPat {sourceSpan = _, scon = _, equality, cookedValue}) acc =
        #1 (doExp equality (#1 (doExp cookedValue acc)))
    | doPat (F.VarPat _) acc = acc
    | doPat (F.RecordPat {sourceSpan = _, fields, ellipsis, allFields = _}) acc =
        List.foldl (fn ((_, pat), acc) => doPat pat acc)
          (case ellipsis of
             NONE => acc
           | SOME basePat => doPat basePat acc) fields
  ...
  and doExp (F.PrimExp (primOp, tyargs, args) : F.Exp) acc :
    TypedSyntax.VIdSet.set * F.Exp =
        let
          val (acc, args') =
            List.foldr
              (fn (x, (acc, xs)) => let val (acc, x) = doExp x acc
                                    in (acc, x :: xs)
                                    end) (acc, []) args
        in
          (acc, F.PrimExp (primOp, tyargs, args'))
        end
    | doExp (exp as F.VarExp vid) acc =
        (TypedSyntax.VIdSet.add (acc, vid), exp)
    | doExp (F.RecordExp fields) acc =
        let
          val (acc, fields) =
            List.foldr
              (fn ((label, exp), (acc, xs)) =>
                 let val (acc, exp) = doExp exp acc
                 in (acc, (label, exp) :: xs)
                 end) (acc, []) fields
        in
          (acc, F.RecordExp fields)
        end
    | doExp (F.LetExp (decs, exp)) acc =
        let
          val (used, exp) = doExp exp TypedSyntax.VIdSet.empty
          val (used', decs) =
            List.foldr
              (fn (dec, (used, decs)) =>
                 let val (used', decs') = doDec (used, dec)
                 in (used', decs' @ decs)
                 end) (used, []) decs
        in
          (TypedSyntax.VIdSet.union (acc, used'), F.LetExp (decs, exp))
        end
    | doExp (F.AppExp (exp1, exp2)) acc =
        let
          val (used, exp1) = doExp exp1 acc
          val (used', exp2) = doExp exp2 used
        in
          (used', F.AppExp (exp1, exp2))
        end
    ...
    | doExp (F.IfThenElseExp (exp1, exp2, exp3)) acc =
        let
          val (used1, exp1) = doExp exp1 acc
          val (used2, exp2) = doExp exp2 used1
          val (used3, exp3) = doExp exp3 used2
        in
          (used3, F.IfThenElseExp (exp1, exp2, exp3))
        end
    | doExp
        (F.CaseExp
           {sourceSpan, subjectExp, subjectTy, matches, matchType, resultTy})
        acc =
        let
          val (used, subjectExp) = doExp subjectExp acc
          val (_, matches) =
            List.foldr
              (fn ((pat, exp), (used, matches)) =>
    ...
    | doExp (F.TyAbsExp (tyvar, kind, exp)) acc =
        let val (used, exp) = doExp exp acc
        in (used, F.TyAbsExp (tyvar, kind, exp))
        end
    | doExp (F.TyAppExp (exp, ty)) acc =
        let val (used, exp) = doExp exp acc
        in (used, F.TyAppExp (exp, ty))
        end
    | doExp (F.PackExp {payloadTy, exp, packageTy}) acc =
        let
          val (used, exp) = doExp exp acc
        in
          ( used
          , F.PackExp {payloadTy = payloadTy, exp = exp, packageTy = packageTy}
          )
        end
    | doExp (exp as F.BogusExp _) acc = (acc, exp)
    | doExp (exp as F.ExitProgram) acc = (acc, exp)
    | doExp (F.ExportValue exp) acc =
        let val (used, exp) = doExp exp acc
        in (used, F.ExportValue exp)
        end
    | doExp (F.ExportModule entities) acc =
        let
          val (acc, entities') =
            Vector.foldr
              (fn ((name, exp), (acc, xs)) => let val (acc, exp) = doExp exp acc
                                              in (acc, (name, exp) :: xs)
                                              end) (acc, []) entities
        in
          (acc, F.ExportModule (Vector.fromList entities'))
        end
  and doIgnoredExpAsExp exp acc =
    let val (used, exps) = doIgnoredExp exp acc
  ...
  and doIgnoredExp (exp as F.PrimExp (primOp, _, args)) acc =
        if isDiscardablePrimOp primOp then
          List.foldr
            (fn (x, (acc, xs)) => let val (acc, ys) = doIgnoredExp x acc
                                  in (acc, ys @ xs)
                                  end) (acc, []) args
        else
          let val (used, exp) = doExp exp acc
    ...
    | doIgnoredExp (F.RecordExp fields) acc =
        List.foldr
          (fn ((_, exp), (acc, xs)) => let val (acc, ys) = doIgnoredExp exp acc
                                       in (acc, ys @ xs)
                                       end) (acc, []) fields
    | doIgnoredExp (F.LetExp (decs, exp)) acc =
        let
          val (used, exp) = doIgnoredExpAsExp exp TypedSyntax.VIdSet.empty
          val (used, decs) =
            List.foldr
              (fn (dec, (used, decs)) =>
                 let val (used, decs') = doDec (used, dec)
    ...
    | doIgnoredExp (F.AppExp (exp1, exp2)) acc =
        let
          val (used1, exp1) = doExp exp1 acc
          val (used2, exp2) = doExp exp2 used1
        in
          (used2, [F.AppExp (exp1, exp2)])
        end
    | doIgnoredExp (F.HandleExp {body, exnName, handler}) acc =
        let
          ...
          val (used1, body) = doIgnoredExpAsExp body
            (TypedSyntax.VIdSet.union (acc, used2))
        in
          case body of
            F.RecordExp [] => (used1, [])
          | _ =>
        ...
        let
          ...
          val (used, matches) =
            List.foldr
              (fn ((pat, exp), (used, matches)) =>
                 let
                   val (used', exp) = doIgnoredExpAsExp exp used
                   val used'' = doPat pat used'
                 in
                   (used'', (pat, exp) :: matches)
                 end) (used, []) matches
        in
          ( used
          , [F.CaseExp
    ...
    | doIgnoredExp (F.ProjectionExp {label = _, record, fieldTypes = _}) acc =
        doIgnoredExp record acc
    | doIgnoredExp (F.TyAbsExp (tyvar, kind, exp)) acc =
        let
          val (used, exp) = doIgnoredExpAsExp exp acc (* should be pure *)
        in
          case exp of
            F.RecordExp [] => (used, [])
          | exp => (used, [F.TyAbsExp (tyvar, kind, exp)])
        end
    | doIgnoredExp (F.TyAppExp (exp, ty)) acc =
        let
          val (used, exp) = doIgnoredExpAsExp exp acc
        in
          case exp of
            F.RecordExp [] => (used, [])
          | exp => (used, [F.TyAppExp (exp, ty)])
        end
    | doIgnoredExp (F.PackExp {payloadTy, exp, packageTy}) acc =
        let
          val (used, exp) = doIgnoredExpAsExp exp acc
        in
          case exp of
          ...
          | exp =>
              ( used
              , [F.PackExp
                   {payloadTy = payloadTy, exp = exp, packageTy = packageTy}]
              )
        end
    | doIgnoredExp (exp as F.BogusExp _) acc = (acc, [exp])
    | doIgnoredExp (exp as F.ExitProgram) acc = (acc, [exp])
    | doIgnoredExp (F.ExportValue exp) acc =
        let val (used, exp) = doExp exp acc
        in (used, [F.ExportValue exp])
        end
    | doIgnoredExp (F.ExportModule entities) acc =
        let
          val (acc, entities') =
            Vector.foldr
              (fn ((name, exp), (acc, xs)) => let val (acc, exp) = doExp exp acc
  ...
  and doDec (used: TypedSyntax.VIdSet.set, F.ValDec (vid, optTy, exp)) :
    TypedSyntax.VIdSet.set * F.Dec list =
    ...
    | doDec (used, F.RecValDec valbinds) =
        let
          val bound =
            List.foldl
              (fn ((vid, _, _), acc) => TypedSyntax.VIdSet.add (acc, vid))
              TypedSyntax.VIdSet.empty valbinds
        in
          if TypedSyntax.VIdSet.disjoint (used, bound) then
            (used, []) (* RHS should be fn _ => _, and therefore discardable *)
          else
            let
              val (used, valbinds) =
                List.foldr
                  (fn ((vid, ty, exp), (used, valbinds)) =>
                     let val (used', exp) = doExp exp used
                     in (used', (vid, ty, exp) :: valbinds)
                     end) (used, []) valbinds
            in
              (used, [F.RecValDec valbinds])
            end
        end
    ...
    | doDec (used, dec as F.ExceptionDec {name = _, tagName, payloadTy = _}) =
        if TypedSyntax.VIdSet.member (used, tagName) then (used, [dec])
        else (used, [])
    | doDec (used, F.ESImportDec {pure, specs, moduleName}) =
        let
          val specs =
            List.filter
              (fn (_, vid, _) => TypedSyntax.VIdSet.member (used, vid)) specs
        in
          ( used

...
struct
         ...
         in
               ...
               let
                 val exp =
                   if Numeric.Notation.isNegative x then
                     case
                       ( #targetLuaVersion ctx
                       , Numeric.Notation.isNegativeZero x
                       )
                     of
                       (LUAJIT, true) =>
           ...
           | C.ValDec
               ...
               let
                 fun doFields (_, []) = []
                   | doFields (i, y :: ys) =
                       (L.IntKey i, doValue ctx y) :: doFields (i + 1, ys)
               in
                 pure
                   ( result
           ...
           | C.ValDec
               { exp =
                   C.PrimOp
                     { primOp = F.DataTagAsStringOp info
                     , tyargs = _
                     , args = [exp]
                     }
               , results = [result]
               ...
               (case #representation info of
                  Syntax.REP_BOXED =>
                    pure (result, L.IndexExp
                      (doValue ctx exp, L.ConstExp (L.LiteralString "tag")))
           ...
           | C.ValDec
               ...
               (case #representation info of
                  Syntax.REP_BOXED =>
                    pure (result, L.IndexExp
                      (doValue ctx exp, L.ConstExp (L.LiteralString "payload")))
           ...
           | C.ValDec
               ...
               in
                 ...
                 | Syntax.REP_ENUM =>
                     pure (result, L.ConstExp (L.LiteralString tag))
           ...
           | C.ValDec
               ...
               let
                   ...
                   | _ =>
                       raise CodeGenError
                         ("primop " ^ Primitives.toString prim
                          ^ ": invalid number of arguments")
                 fun doBinary f =
                   doBinaryRaw (fn (a, b) => f (doValue ctx a, doValue ctx b))
                 ...
                 fun doTernary f =
                   case args of
                     [a, b, c] =>
                       f (doValue ctx a, doValue ctx b, doValue ctx c)
                   | _ =>
                       raise CodeGenError
                         ("primop " ^ Primitives.toString prim
                          ^ ": invalid number of arguments")
               in
                     ...
                     (case (#targetLuaVersion ctx, i) of
                      ...
                      | (LUAJIT, Primitives.I54) =>
                          doUnaryExp
                            ( fn a =>
                                L.BinExp
                                  (L.MINUS, L.ConstExp (L.Numeral "0"), a)
                            , PURE
                 ...
                 | Primitives.Word_div_unchecked w =>
                     (case (#targetLuaVersion ctx, w) of
                        (LUA5_3, Primitives.WORD) =>
                          doBinaryExpRaw
                            ( fn (a, b as C.WordConst (Primitives.WORD, b')) =>
                                if b' mod 2 = 0 then
                                  let
                                  ...
                                  L.CallExp
                                    ( L.VarExp (L.PredefinedId "_Word_div")
                                    , vector [doValue ctx a, doValue ctx b]
                                    )
                               | (a, b) =>
                                L.CallExp
                                  ( L.VarExp (L.PredefinedId "_Word_div")
                 ...
                 | Primitives.String_str =>
                     doUnaryExp
                       ( fn a =>
                           L.CallExp
                             ( L.VarExp (L.PredefinedId "string_char")
                             , vector [a]
                             )
                       , PURE
                       )
                 | Primitives.Vector_length _ =>
                     doUnaryExp
                       ( fn a =>
                           L.IndexExp (a, L.ConstExp (L.LiteralString "n"))
                       , PURE
                       )
           ...
           | C.ValDec
               { exp =
                   C.PrimOp {primOp = F.LuaCallOp, tyargs = _, args = f :: args}
               , results = [result]
               } =>
               (case result of
                  NONE =>
                    doDecs
                      ...
                      , L.CallStat
                          ( doValue ctx f
                          , Vector.map (doValue ctx) (vector args)
                          ) :: revStats
                      )
                | SOME result =>
                    doDecs
           ...
           | C.ValDec
               { exp = C.PrimOp {primOp = F.JsCallOp, tyargs = _, args = _}
               , results = _
               } =>
               raise CodeGenError "JsCallOp is not supported on Lua backend"
           | C.ValDec
               { exp = C.PrimOp {primOp = F.JsMethodOp, tyargs = _, args = _}
               , results = _
               } =>
               raise CodeGenError "JsMethodOp is not supported on Lua backend"
           | C.ValDec
               { exp = C.PrimOp {primOp = F.JsNewOp, tyargs = _, args = _}
               , results = _
               ...
               let
                 val fields =
                   Syntax.LabelMap.foldri
                     (fn (label, v, acc) =>
                        (LabelToTableKey label, doValue ctx v) :: acc) [] fields
               in
                 pure (result, L.TableExp (vector fields))
               end
           | C.ValDec {exp = C.ExnTag {name, payloadTy = _}, results = [result]} =>
               discardable (result, L.TableExp (vector
                 [(L.IntKey 1, L.ConstExp (L.LiteralString name))]))
           | C.ValDec
                 ...
                 val label =
                   case label of
                     Syntax.NumericLabel n =>
                       L.ConstExp (L.Numeral (Int.toString n))
                   | Syntax.IdentifierLabel s => L.ConstExp (L.LiteralString s)
               in
                 pure (result, L.IndexExp (doValue ctx record, label))
               end
           | C.ValDec
               ...
               let
                 val env' =
                   {continuations = C.CVarMap.singleton (contParam, RETURN)}
               in
                 pure
                   ( result
                   , L.FunctionExp
           ...
           | C.RecContDec defs =>
               let
                 datatype init =
                   INIT_WITH_VALUES of C.CVar * C.Value list
                 | NO_INIT
                 val init =
                   case (decs, finalExp) of
                 ...
                 val initAndRest =
                   case init of
                     INIT_WITH_VALUES (initCont, args) =>
                       let
                         val args' = List.map (doValue ctx) args
                         val decs' =
                           if maxParams > 0 then
                             [L.LocalStat
  ...
  and doCExp
        (ctx: Context, env: Env, defaultCont: C.CVar option, C.Let {decs, cont}) =
    ...
    | doCExp (ctx, env, _, C.App {applied, cont, args}) =
        (case C.CVarMap.find (#continuations env, cont) of
         ...
         | SOME RETURN =>
             [L.ReturnStat (vector
                [L.CallExp
                   (doValue ctx applied, Vector.map (doValue ctx) (vector args))])] (* tail call *)
         | NONE => raise CodeGenError "undefined continuation")
    | doCExp (ctx, env, defaultCont, C.AppCont {applied, args}) =
        applyCont (ctx, env, defaultCont, applied, List.map (doValue ctx) args)
    | doCExp (ctx, env, defaultCont, C.If {cond, thenCont, elseCont}) =
        let
          fun containsNestedBlock (C.Let {decs, cont}) =
          ...
          and containsNestedBlockDec (C.ValDec _) = false
            ...
            | containsNestedBlockDec (C.ESImportDec _) =
                false (* cannot occur *)
        in

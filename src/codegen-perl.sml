(*
 * Copyright (c) 2023 ARATA Mizuki
 * This file is part of LunarML.
 *)
structure CodeGenPl :>
sig
  exception CodeGenError of string
  datatype code_style = DIRECT_STYLE | CPS
  type Context =
    { nextPlId: int ref
    , style: code_style
    , contEscapeMap: CpsAnalyze.cont_map
    , imports: ({specs: (string * PlSyntax.Id) list, moduleName: string} list) ref
    }
  val doProgramDirect: Context -> CSyntax.CVar -> CSyntax.CExp -> PlSyntax.Block
  val doProgramDirectDefaultExport: Context
                                    -> CSyntax.CVar
                                    -> CSyntax.CExp
                                    -> PlSyntax.Block
  val doProgramDirectNamedExport: Context
                                  -> CSyntax.CVar
                                  -> CSyntax.CExp
                                  -> string vector
                                  -> PlSyntax.Block
  val doProgramCPS: Context -> CSyntax.CVar -> CSyntax.CExp -> PlSyntax.Block
  val doProgramCPSDefaultExport: Context
                                 -> CSyntax.CVar
                                 -> CSyntax.CExp
                                 -> PlSyntax.Block
  val doProgramCPSNamedExport: Context
                               -> CSyntax.CVar
                               -> CSyntax.CExp
                               -> string vector
                               -> PlSyntax.Block
end =
struct
  exception CodeGenError of string
  (* Mapping of types:
   * int -> 54-bit signed integer, as a subset of 64-bit floating-point number
   * word -> 32-bit unsigned integer, as a subset of 64-bit floating-point number (excluding negative zero)
   * real -> number (64-bit floating-point number)
   * string -> immutable Uint8Array
   * char -> 8-bit unsigned integer, as a subset of 64-bit floating-point number (excluding negative zero)
   * String16.string (WideString.string) -> 16-bit string
   * Char16.char (WideChar.char) -> 16-bit unsigned integer, as a subset of 64-bit floating-point number (excluding negative zero)
   * exn -> object
   * bool -> boolean
   * ref -> [ <mutable> ]
   * list -> null | [<head>, <tail>]
   * tuple -> immutable Array
   * non-tuple record -> immutable object, with integer index starting with 0
   * vector -> immutable Array
   * array -> mutable Array
   * function -> function with optional _MLTAIL_ field (direct style)
   *)
  structure F = FSyntax
  structure C = CSyntax
  structure PL = PlSyntax

  fun ConstStat (vid, exp) =
    PL.ConstStat (vector [(vid, exp)])

  val builtinsDirect =
    let
      open InitialEnv
    in
      List.foldl
        (fn ((vid, name), map) => TypedSyntax.VIdMap.insert (map, vid, name))
        TypedSyntax.VIdMap.empty
        [ (* exn *) (VId_Match, "_Match")
        , (VId_Bind, "_Bind")
        , (VId_Div, "_Div")
        , (VId_Overflow, "_Overflow")
        , (VId_Size, "_Size")
        , (VId_Subscript, "_Subscript")
        , (VId_Fail, "_Fail")
        , (VId_Match_tag, "_Match_tag")
        , (VId_Bind_tag, "_Bind_tag")
        , (VId_Div_tag, "_Div_tag")
        , (VId_Overflow_tag, "_Overflow_tag")
        , (VId_Size_tag, "_Size_tag")
        , (VId_Subscript_tag, "_Subscript_tag")
        , (VId_Fail_tag, "_Fail_tag")
        , (VId_exnName, "_exnName")
        (* string *)
        , (VId_String_concat, "_String_concat")
        , (VId_String_concatWith, "_String_concatWith")
        , (VId_String_implode, "_String_implode")
        , (VId_String_translate, "_String_translate")
        (* real *)
        , (VId_Real_abs, "Math_abs") (* PL Math.abs *)
        (* Vector and Array *)
        , (VId_Vector_tabulate, "_VectorOrArray_tabulate")
        , (VId_Vector_concat, "_Vector_concat")
        , (VId_Vector_fromList, "_VectorOrArray_fromList")
        , (VId_Array_array, "_Array_array")
        , (VId_Array_fromList, "_VectorOrArray_fromList")
        , (VId_Array_tabulate, "_VectorOrArray_tabulate")
        (* PL interface *)
        , (VId_Perl_undefined, "undefined")
        , (VId_Perl_null, "null")
        , (VId_Perl_function, "_function")
        , (VId_Perl_encodeUtf8, "_encodeUtf8")
        , (VId_Perl_decodeUtf8, "_decodeUtf8")
        ]
    end
  val builtinsCPS =
    let
      open InitialEnv
    in
      List.foldl
        (fn ((vid, name), map) => TypedSyntax.VIdMap.insert (map, vid, name))
        TypedSyntax.VIdMap.empty
        [ (* exn *) (VId_Match, "_Match")
        , (VId_Bind, "_Bind")
        , (VId_Div, "_Div")
        , (VId_Overflow, "_Overflow")
        , (VId_Size, "_Size")
        , (VId_Subscript, "_Subscript")
        , (VId_Fail, "_Fail")
        , (VId_Match_tag, "_Match_tag")
        , (VId_Bind_tag, "_Bind_tag")
        , (VId_Div_tag, "_Div_tag")
        , (VId_Overflow_tag, "_Overflow_tag")
        , (VId_Size_tag, "_Size_tag")
        , (VId_Subscript_tag, "_Subscript_tag")
        , (VId_Fail_tag, "_Fail_tag")
        , (VId_exnName, "_exnName")
        (* string *)
        , (VId_String_concat, "_String_concat")
        , (VId_String_concatWith, "_String_concatWith")
        , (VId_String_implode, "_String_implode")
        (* real *)
        , (VId_Real_abs, "_Real_abs")
        (* Vector and Array *)
        , (VId_Vector_concat, "_Vector_concat")
        , (VId_Vector_fromList, "_VectorOrArray_fromList")
        , (VId_Array_array, "_Array_array")
        , (VId_Array_fromList, "_VectorOrArray_fromList")
        (* PL interface *)
        , (VId_Perl_undefined, "undefined")
        , (VId_Perl_null, "null")
        , (VId_Perl_function, "_function")
        , (VId_Perl_encodeUtf8, "_encodeUtf8")
        , (VId_Perl_decodeUtf8, "_decodeUtf8")
        (* extra *)
        , (VId_DelimCont_pushPrompt, "_pushPrompt")
        , (VId_DelimCont_withSubCont, "_withSubCont")
        , (VId_DelimCont_pushSubCont, "_pushSubCont")
        , (VId_DelimCont_topLevel, "_topLevel")
        ]
    end

  datatype code_style = DIRECT_STYLE | CPS
  type Context =
    { nextPlId: int ref
    , style: code_style
    , contEscapeMap: CpsAnalyze.cont_map
    , imports: ({specs: (string * PL.Id) list, moduleName: string} list) ref
    }

  fun VIdToPl (ctx: Context) (vid as TypedSyntax.MkVId (name, n)) =
    if n < 0 then
      let
        val builtins =
          case #style ctx of
            DIRECT_STYLE => builtinsDirect
          | CPS => builtinsCPS
      in
        case TypedSyntax.VIdMap.find (builtins, vid) of
          NONE =>
            raise Fail
              ("the built-in symbol " ^ name ^ "@" ^ Int.toString n
               ^ " is not supported by Perl backend")
        | SOME perlName => PlSyntax.PredefinedId perlName
      end
    else
      PlSyntax.UserDefinedId vid

  datatype cont_type =
    BREAK_TO of
      { label: PL.Id
      , which: (PL.Id * PL.PlConst) option
      , params: (PL.Id option) list
      }
  | CONTINUE_TO of
      { label: PL.Id
      , which: (PL.Id * PL.PlConst) option
      , params: (PL.Id option) list
      }
  | TAILCALL of C.CVar (* continuation passing style *)
  | RETURN_TRAMPOLINE (* direct style *)
  | RETURN_SIMPLE (* direct style *)
  type Env =
    { continuations: cont_type C.CVarMap.map
    , subst: PL.Exp TypedSyntax.VIdMap.map
    }

  fun doValue (ctx, env: Env) (C.Var vid) =
        (case TypedSyntax.VIdMap.find (#subst env, vid) of
           SOME e => e
         | NONE =>
             case VIdToPl ctx vid of
               PL.PredefinedId "null" => PL.ConstExp PL.Null
             | PL.PredefinedId "false" => PL.ConstExp PL.False
             | PL.PredefinedId "true" => PL.ConstExp PL.True
             | id => PL.VarExp id)
    | doValue _ C.Unit = PL.UndefinedExp
    | doValue _ C.Nil = PL.ConstExp PL.Null (* empty list *)
    | doValue _ (C.BoolConst false) = PL.ConstExp PL.False
    | doValue _ (C.BoolConst true) = PL.ConstExp PL.True
    | doValue _ (C.IntConst (Primitives.INT, _)) =
        raise Fail "NativeIntConst is not supported by Perl backend"
    | doValue _ (C.IntConst (Primitives.I32, x)) =
        if x < 0 then
          PL.UnaryExp (PL.NEGATE, PL.ConstExp (PL.Numeral
            (LargeInt.toString (~x))))
        else
          PL.ConstExp (PL.Numeral (LargeInt.toString x))
    | doValue _ (C.IntConst (Primitives.I54, x)) =
        if x < 0 then
          PL.UnaryExp (PL.NEGATE, PL.ConstExp (PL.Numeral
            (LargeInt.toString (~x))))
        else
          PL.ConstExp (PL.Numeral (LargeInt.toString x))
    | doValue _ (C.IntConst (Primitives.I64, x)) =
        if x < 0 then
          PL.UnaryExp (PL.NEGATE, PL.ConstExp (PL.Numeral
            (LargeInt.toString (~x) ^ "n")))
        else
          PL.ConstExp (PL.Numeral (LargeInt.toString x ^ "n"))
    | doValue _ (C.IntConst (Primitives.INT_INF, x)) =
        if x < 0 then
          PL.UnaryExp (PL.NEGATE, PL.ConstExp (PL.Numeral
            (LargeInt.toString (~x) ^ "n")))
        else
          PL.ConstExp (PL.Numeral (LargeInt.toString x ^ "n"))
    | doValue _ (C.WordConst (Primitives.WORD, _)) =
        raise Fail "NativeWordConst is not supported by Perl backend"
    | doValue _ (C.WordConst (Primitives.W32, x)) =
        PL.ConstExp (PL.Numeral ("0x" ^ LargeInt.fmt StringCvt.HEX x))
    | doValue _ (C.WordConst (Primitives.W64, x)) =
        PL.ConstExp (PL.Numeral ("0x" ^ LargeInt.fmt StringCvt.HEX x ^ "n"))
    | doValue _ (C.CharConst x) =
        PL.ConstExp (PL.Numeral (Int.toString (ord x)))
    | doValue _ (C.Char16Const x) =
        PL.ConstExp (PL.Numeral (Int.toString x))
    | doValue _ (C.StringConst x) =
        PL.MethodExp
          ( PL.VarExp (PL.PredefinedId "Uint8Array")
          , "of"
          , Vector.map (PL.ConstExp o PL.Numeral o Int.toString o Char.ord)
              (Vector.fromList (String.explode x))
          )
    | doValue _ (C.String16Const x) =
        PL.ConstExp (PL.WideString x)

  fun CVarToId v =
    TypedSyntax.MkVId ("cont", C.CVar.toInt v)
  fun CVarToPl v =
    PL.UserDefinedId (CVarToId v)
  fun doCVar v =
    PL.VarExp (CVarToPl v)

  fun LabelToObjectKey (Syntax.NumericLabel n) =
        PlSyntax.IntKey (n - 1)
    | LabelToObjectKey (Syntax.IdentifierLabel s) = PlSyntax.StringKey s

  datatype purity = PURE | DISCARDABLE | IMPURE

  fun genSymNamed (ctx: Context, name) =
    let
      val n = !(#nextPlId ctx)
      val _ = #nextPlId ctx := n + 1
    in
      TypedSyntax.MkVId (name, n)
    end
  fun genSym ctx = genSymNamed (ctx, "tmp")
  (*
  fun genExnContSym (ctx : Context) = let val n = !(#nextPlId ctx)
                                          val _ = #nextPlId ctx := n + 1
                                      in CSyntax.CVar.fromInt n
                                      end
  *)

  fun applyCont (_: Context, env: Env, cont, args) =
    case C.CVarMap.find (#continuations env, cont) of
      SOME (BREAK_TO {label, which, params}) =>
        let
          val (params', args') =
            ListPair.foldrEq
              (fn (SOME p, a, (pp, aa)) => (p :: pp, a :: aa)
                | (NONE, _, acc) => acc) ([], []) (params, args)
          val setWhich =
            case which of
              NONE => []
            | SOME (whichVar, whichVal) =>
                [PL.AssignStat (PL.VarExp whichVar, PL.ConstExp whichVal)]
        in
          PL.MultiAssignStat (List.map PL.VarExp params', args') @ setWhich
          @ [PL.BreakStat (SOME label)]
        end
    | SOME (CONTINUE_TO {label, which, params}) =>
        let
          val (params', args') =
            ListPair.foldrEq
              (fn (SOME p, a, (pp, aa)) => (p :: pp, a :: aa)
                | (NONE, _, acc) => acc) ([], []) (params, args)
          val setWhich =
            case which of
              NONE => []
            | SOME (whichVar, whichVal) =>
                [PL.AssignStat (PL.VarExp whichVar, PL.ConstExp whichVal)]
        in
          PL.MultiAssignStat (List.map PL.VarExp params', args') @ setWhich
          @ [PL.ContinueStat (SOME label)]
        end
    | SOME (TAILCALL k) =>
        [PL.ReturnStat (SOME (PL.ArrayExp (vector
           [PL.ConstExp PL.False, doCVar k, PL.ArrayExp (vector args)])))] (* continuation passing style *)
    | SOME RETURN_TRAMPOLINE =>
        [PL.ReturnStat (SOME (PL.ArrayExp (vector
           (PL.ConstExp PL.True :: args))))] (* direct style *)
    | SOME RETURN_SIMPLE =>
        (case args of
           [v] => [PL.ReturnStat (SOME v)] (* direct style *)
         | _ => raise CodeGenError "invalid return arity")
    | NONE => raise CodeGenError "undefined continuation"
  (*: val doDecs : Context * Env * C.Dec list * C.CExp * PL.Stat list -> PL.Stat list *)
  fun doDecs (ctx, env, decs, finalExp, revStats) =
    (case decs of
       [] => List.revAppend (revStats, doCExp ctx env finalExp)
     | dec :: decs =>
         let
           fun pure (NONE, _) =
                 doDecs (ctx, env, decs, finalExp, revStats)
             | pure (SOME result, exp) =
                 doDecs
                   ( ctx
                   , env
                   , decs
                   , finalExp
                   , ConstStat (result, exp) :: revStats
                   )
           fun discardable (NONE, _) =
                 doDecs (ctx, env, decs, finalExp, revStats)
             | discardable (SOME result, exp) =
                 doDecs
                   ( ctx
                   , env
                   , decs
                   , finalExp
                   , ConstStat (result, exp) :: revStats
                   )
           fun impure (NONE, exp) =
                 doDecs (ctx, env, decs, finalExp, PL.ExpStat exp :: revStats)
             | impure (SOME result, exp) =
                 doDecs
                   ( ctx
                   , env
                   , decs
                   , finalExp
                   , ConstStat (result, exp) :: revStats
                   )
         in
           case dec of
             C.ValDec
               { exp = C.PrimOp {primOp = F.RealConstOp x, tyargs = _, args = _}
               , results = [result]
               } =>
               let
                 val exp =
                   let
                     val y =
                       Numeric.toDecimal
                         { nominal_format = Numeric.binary64
                         , target_format = Numeric.binary64
                         } x
                   (* Perl does not support hexadecimal floating-point literals *)
                   in
                     case y of
                       SOME z =>
                         if Numeric.Notation.isNegative z then
                           PL.UnaryExp (PL.NEGATE, PL.ConstExp (PL.Numeral
                             (Numeric.Notation.toString "-"
                                (Numeric.Notation.abs z))))
                         else
                           PL.ConstExp (PL.Numeral
                             (Numeric.Notation.toString "-" z))
                     | NONE =>
                         raise CodeGenError
                           "the hexadecimal floating-point value cannot be represented as a 64-bit floating-point number"
                   end
               in
                 pure (result, exp)
               end
           | C.ValDec
               { exp = C.PrimOp {primOp = F.ListOp, tyargs = _, args = []}
               , results = [result]
               } => pure (result, PL.ConstExp PL.Null)
           | C.ValDec
               { exp = C.PrimOp {primOp = F.ListOp, tyargs = _, args = xs}
               , results = [result]
               } =>
               pure
                 ( result
                 , PL.CallExp (PL.VarExp (PL.PredefinedId "_list"), vector
                     [PL.ArrayExp (Vector.map (doValue (ctx, env)) (vector xs))])
                 )
           | C.ValDec
               { exp = C.PrimOp {primOp = F.VectorOp, tyargs = _, args = xs}
               , results = [result]
               } =>
               pure (result, PL.ArrayExp
                 (Vector.map (doValue (ctx, env)) (vector xs)))
           | C.ValDec
               { exp =
                   C.PrimOp
                     { primOp = F.DataTagAsString16Op info
                     , tyargs = _
                     , args = [exp]
                     }
               , results = [result]
               } =>
               (case #representation info of
                  Syntax.REP_BOXED =>
                    pure (result, PL.IndexExp
                      ( doValue (ctx, env) exp
                      , PL.ConstExp (PL.asciiStringAsWide "tag")
                      ))
                | Syntax.REP_ENUM => pure (result, doValue (ctx, env) exp)
                | _ =>
                    raise CodeGenError
                      "unexpected datatype representation for DataTagAsString16Op")
           | C.ValDec
               { exp =
                   C.PrimOp
                     {primOp = F.DataPayloadOp info, tyargs = _, args = [exp]}
               , results = [result]
               } =>
               (case #representation info of
                  Syntax.REP_BOXED =>
                    pure (result, PL.IndexExp
                      ( doValue (ctx, env) exp
                      , PL.ConstExp (PL.asciiStringAsWide "payload")
                      ))
                | Syntax.REP_ALIAS => pure (result, doValue (ctx, env) exp)
                | _ =>
                    raise CodeGenError
                      "unexpected datatype representation for DataPayloadOp")
           | C.ValDec
               { exp =
                   C.PrimOp {primOp = F.ExnPayloadOp, tyargs = _, args = [exp]}
               , results = [result]
               } =>
               pure (result, PL.IndexExp
                 ( doValue (ctx, env) exp
                 , PL.ConstExp (PL.asciiStringAsWide "payload")
                 ))
           | C.ValDec
               { exp =
                   C.PrimOp
                     {primOp = F.ConstructValOp info, tyargs = _, args = []}
               , results = [result]
               } =>
               let
                 val tag = #tag info
               in
                 case #representation info of
                   Syntax.REP_BOXED =>
                     pure (result, PL.ObjectExp (vector
                       [( PL.StringKey "tag"
                        , PL.ConstExp (PL.asciiStringAsWide tag)
                        )]))
                 | Syntax.REP_ENUM =>
                     pure (result, PL.ConstExp (PL.asciiStringAsWide tag))
                 | Syntax.REP_UNIT => pure (result, PL.ConstExp PL.Null)
                 | _ =>
                     raise CodeGenError
                       "unexpected datatype representation for ConstructValOp"
               end
           | C.ValDec
               { exp =
                   C.PrimOp
                     { primOp = F.ConstructValWithPayloadOp info
                     , tyargs = _
                     , args = [payload]
                     }
               , results = [result]
               } =>
               let
                 val tag = #tag info
                 val payload = doValue (ctx, env) payload
               in
                 case #representation info of
                   Syntax.REP_BOXED =>
                     pure (result, PL.ObjectExp (vector
                       [ ( PL.StringKey "tag"
                         , PL.ConstExp (PL.asciiStringAsWide tag)
                         )
                       , (PL.StringKey "payload", payload)
                       ]))
                 | Syntax.REP_ALIAS => pure (result, payload)
                 | _ =>
                     raise CodeGenError
                       "unexpected datatype representation for ConstructValWithPayloadOp"
               end
           | C.ValDec
               { exp =
                   C.PrimOp
                     {primOp = F.ConstructExnOp, tyargs = _, args = [tag]}
               , results = [result]
               } =>
               let val tag = doValue (ctx, env) tag
               in pure (result, PL.NewExp (tag, vector []))
               end
           | C.ValDec
               { exp =
                   C.PrimOp
                     { primOp = F.ConstructExnWithPayloadOp
                     , tyargs = _
                     , args = [tag, payload]
                     }
               , results = [result]
               } =>
               let
                 val tag = doValue (ctx, env) tag
                 val payload = doValue (ctx, env) payload
               in
                 pure (result, PL.NewExp (tag, vector [payload]))
               end
           | C.ValDec
               { exp =
                   C.PrimOp
                     { primOp =
                         F.RaiseOp
                           (_ (* span as { start as { file, line, column }, ... } *))
                     , tyargs = _
                     , args = [exp]
                     }
               , results = _
               } =>
               List.rev
                 (PL.ThrowStat (doValue (ctx, env) exp) :: revStats) (* TODO: location information *)
           | C.ValDec
               { exp = C.PrimOp {primOp = F.PrimCall prim, tyargs = _, args}
               , results
               } =>
               let
                 fun action ([], stmt) =
                       doDecs (ctx, env, decs, finalExp, stmt :: revStats)
                   | action (_ :: _, stmt) =
                       raise CodeGenError
                         ("primop " ^ Primitives.toString prim
                          ^ ": unexpected number of results for an action")
                 fun doNullary f =
                   case args of
                     [] => f ()
                   | _ =>
                       raise CodeGenError
                         ("primop " ^ Primitives.toString prim
                          ^ ": invalid number of arguments")
                 fun doNullaryExp (f, purity) =
                   doNullary (fn () =>
                     case results of
                       [result] =>
                         (case purity of
                            PURE => pure (result, f ())
                          | DISCARDABLE => discardable (result, f ())
                          | IMPURE => impure (result, f ()))
                     | _ => raise CodeGenError "unexpected number of results")
                 fun doUnary f =
                   case args of
                     [a] => f (doValue (ctx, env) a)
                   | _ =>
                       raise CodeGenError
                         ("primop " ^ Primitives.toString prim
                          ^ ": invalid number of arguments")
                 fun doUnaryExp (f, purity) =
                   doUnary (fn a =>
                     case results of
                       [result] =>
                         (case purity of
                            PURE => pure (result, f a)
                          | DISCARDABLE => discardable (result, f a)
                          | IMPURE => impure (result, f a))
                     | _ => raise CodeGenError "unexpected number of results")
                 fun doBinary f =
                   case args of
                     [a, b] => f (doValue (ctx, env) a, doValue (ctx, env) b)
                   | _ =>
                       raise CodeGenError
                         ("primop " ^ Primitives.toString prim
                          ^ ": invalid number of arguments")
                 fun doBinaryExp (f, purity) =
                   doBinary (fn (a, b) =>
                     case results of
                       [result] =>
                         (case purity of
                            PURE => pure (result, f (a, b))
                          | DISCARDABLE => discardable (result, f (a, b))
                          | IMPURE => impure (result, f (a, b)))
                     | _ => raise CodeGenError "unexpected number of results")
                 fun doBinaryOp (binop, pure) =
                   doBinaryExp (fn (a, b) => PL.BinExp (binop, a, b), pure)
                 fun doTernary f =
                   case args of
                     [a, b, c] =>
                       f ( doValue (ctx, env) a
                         , doValue (ctx, env) b
                         , doValue (ctx, env) c
                         )
                   | _ =>
                       raise CodeGenError
                         ("primop " ^ Primitives.toString prim
                          ^ ": invalid number of arguments")
               in
                 case prim of
                   Primitives.call2 =>
                     doTernary (fn (f, a0, a1) =>
                       case results of
                         [result] =>
                           impure (result, PL.CallExp (f, vector [a0, a1]))
                       | _ => raise CodeGenError "unexpected number of results")
                 | Primitives.List_cons =>
                     doBinaryExp
                       (fn (x, xs) => PL.ArrayExp (vector [x, xs]), PURE)
                 | Primitives.List_null =>
                     doUnaryExp
                       ( fn a => PL.BinExp (PL.EQUAL, a, PL.ConstExp PL.Null)
                       , PURE
                       )
                 | Primitives.List_unsafeHead =>
                     doUnaryExp
                       ( fn xs => PL.IndexExp (xs, PL.ConstExp (PL.Numeral "0"))
                       , PURE
                       )
                 | Primitives.List_unsafeTail =>
                     doUnaryExp
                       ( fn xs => PL.IndexExp (xs, PL.ConstExp (PL.Numeral "1"))
                       , PURE
                       )
                 | Primitives.Ref_ref =>
                     doUnaryExp
                       ( fn x =>
                           (* REPRESENTATION_OF_REF *)
                           PL.ArrayExp (vector [x])
                       , DISCARDABLE
                       )
                 | Primitives.Ref_EQUAL => doBinaryOp (PL.EQUAL, PURE)
                 | Primitives.Ref_set =>
                     doBinary (fn (a, b) =>
                       action (results, PL.AssignStat
                         (PL.IndexExp (a, PL.ConstExp (PL.Numeral "0")), b))) (* REPRESENTATION_OF_REF *)
                 | Primitives.Ref_read =>
                     doUnaryExp
                       ( fn a => PL.IndexExp (a, PL.ConstExp (PL.Numeral "0"))
                       , DISCARDABLE
                       ) (* REPRESENTATION_OF_REF *)
                 | Primitives.Bool_EQUAL => doBinaryOp (PL.EQUAL, PURE)
                 | Primitives.Bool_not =>
                     doUnaryExp (fn a => PL.UnaryExp (PL.NOT, a), PURE)
                 | Primitives.Int_EQUAL _ =>
                     doBinaryOp (PL.EQUAL, PURE) (* Int32, Int54, IntInf *)
                 | Primitives.Int_PLUS Primitives.I32 =>
                     doBinaryExp
                       ( fn (a, b) =>
                           PL.CallExp
                             ( PL.VarExp (PL.PredefinedId "_Int32_add")
                             , vector [a, b]
                             )
                       , IMPURE
                       )
                 | Primitives.Int_PLUS Primitives.I54 =>
                     doBinaryExp
                       ( fn (a, b) =>
                           PL.CallExp
                             ( PL.VarExp (PL.PredefinedId "_Int54_add")
                             , vector [a, b]
                             )
                       , IMPURE
                       )
                 | Primitives.Int_MINUS Primitives.I32 =>
                     doBinaryExp
                       ( fn (a, b) =>
                           PL.CallExp
                             ( PL.VarExp (PL.PredefinedId "_Int32_sub")
                             , vector [a, b]
                             )
                       , IMPURE
                       )
                 | Primitives.Int_MINUS Primitives.I54 =>
                     doBinaryExp
                       ( fn (a, b) =>
                           PL.CallExp
                             ( PL.VarExp (PL.PredefinedId "_Int54_sub")
                             , vector [a, b]
                             )
                       , IMPURE
                       )
                 | Primitives.Int_TIMES Primitives.I32 =>
                     doBinaryExp
                       ( fn (a, b) =>
                           PL.CallExp
                             ( PL.VarExp (PL.PredefinedId "_Int32_mul")
                             , vector [a, b]
                             )
                       , IMPURE
                       )
                 | Primitives.Int_TIMES Primitives.I54 =>
                     doBinaryExp
                       ( fn (a, b) =>
                           PL.CallExp
                             ( PL.VarExp (PL.PredefinedId "_Int54_mul")
                             , vector [a, b]
                             )
                       , IMPURE
                       )
                 | Primitives.Int_div Primitives.I32 =>
                     doBinaryExp
                       ( fn (a, b) =>
                           PL.CallExp
                             ( PL.VarExp (PL.PredefinedId "_Int32_div")
                             , vector [a, b]
                             )
                       , IMPURE
                       )
                 | Primitives.Int_div Primitives.I54 =>
                     doBinaryExp
                       ( fn (a, b) =>
                           PL.CallExp
                             ( PL.VarExp (PL.PredefinedId "_Int54_div")
                             , vector [a, b]
                             )
                       , IMPURE
                       )
                 | Primitives.Int_div_unchecked Primitives.I32 =>
                     doBinaryExp
                       ( fn (a, b) =>
                           PL.ToInt32Exp
                             (PL.MethodExp
                                ( PL.VarExp (PL.PredefinedId "Math")
                                , "floor"
                                , vector [PL.BinExp (PL.DIV, a, b)]
                                ))
                       , PURE
                       )
                 | Primitives.Int_div_unchecked Primitives.I54 =>
                     doBinaryExp
                       ( fn (a, b) =>
                           PL.BinExp
                             ( PL.PLUS
                             , PL.ConstExp (PL.Numeral "0")
                             , PL.MethodExp
                                 ( PL.VarExp (PL.PredefinedId "Math")
                                 , "floor"
                                 , vector [PL.BinExp (PL.DIV, a, b)]
                                 )
                             )
                       , PURE
                       )
                 | Primitives.Int_mod Primitives.I32 =>
                     doBinaryExp
                       ( fn (a, b) =>
                           PL.CallExp
                             ( PL.VarExp (PL.PredefinedId "_Int32_mod")
                             , vector [a, b]
                             )
                       , IMPURE
                       )
                 | Primitives.Int_mod Primitives.I54 =>
                     doBinaryExp
                       ( fn (a, b) =>
                           PL.CallExp
                             ( PL.VarExp (PL.PredefinedId "_Int54_mod")
                             , vector [a, b]
                             )
                       , IMPURE
                       )
                 | Primitives.Int_mod_unchecked Primitives.I32 =>
                     doBinaryExp
                       ( fn (a, b) =>
                           PL.CallExp
                             ( PL.VarExp (PL.PredefinedId "_Int32_mod")
                             , vector [a, b]
                             )
                       , PURE
                       )
                 | Primitives.Int_mod_unchecked Primitives.I54 =>
                     doBinaryExp
                       ( fn (a, b) =>
                           PL.CallExp
                             ( PL.VarExp (PL.PredefinedId "_Int54_mod")
                             , vector [a, b]
                             )
                       , PURE
                       )
                 | Primitives.Int_quot Primitives.I32 =>
                     doBinaryExp
                       ( fn (a, b) =>
                           PL.CallExp
                             ( PL.VarExp (PL.PredefinedId "_Int32_quot")
                             , vector [a, b]
                             )
                       , IMPURE
                       )
                 | Primitives.Int_quot Primitives.I54 =>
                     doBinaryExp
                       ( fn (a, b) =>
                           PL.CallExp
                             ( PL.VarExp (PL.PredefinedId "_Int54_quot")
                             , vector [a, b]
                             )
                       , IMPURE
                       )
                 | Primitives.Int_quot_unchecked Primitives.I32 =>
                     doBinaryExp
                       ( fn (a, b) => PL.ToInt32Exp (PL.BinExp (PL.DIV, a, b))
                       , PURE
                       )
                 | Primitives.Int_quot_unchecked Primitives.I54 =>
                     doBinaryExp
                       ( fn (a, b) =>
                           PL.BinExp
                             ( PL.PLUS
                             , PL.ConstExp (PL.Numeral "0")
                             , PL.MethodExp
                                 ( PL.VarExp (PL.PredefinedId "Math")
                                 , "trunc"
                                 , vector [PL.BinExp (PL.DIV, a, b)]
                                 )
                             )
                       , PURE
                       )
                 | Primitives.Int_rem_unchecked Primitives.I32 =>
                     doBinaryExp
                       ( fn (a, b) => PL.ToInt32Exp (PL.BinExp (PL.MOD, a, b))
                       , PURE
                       )
                 | Primitives.Int_rem_unchecked Primitives.I54 =>
                     doBinaryExp
                       ( fn (a, b) =>
                           PL.BinExp
                             ( PL.PLUS
                             , PL.ConstExp (PL.Numeral "0")
                             , PL.BinExp (PL.MOD, a, b)
                             )
                       , PURE
                       )
                 | Primitives.Int_TILDE Primitives.I32 =>
                     doUnaryExp
                       ( fn a =>
                           PL.CallExp
                             ( PL.VarExp (PL.PredefinedId "_Int32_negate")
                             , vector [a]
                             )
                       , IMPURE
                       )
                 | Primitives.Int_TILDE Primitives.I54 =>
                     doUnaryExp
                       ( fn a =>
                           PL.CallExp
                             ( PL.VarExp (PL.PredefinedId "_Int54_negate")
                             , vector [a]
                             )
                       , IMPURE
                       )
                 | Primitives.Int_TILDE_unchecked Primitives.I32 =>
                     doUnaryExp
                       ( fn a => PL.ToInt32Exp (PL.UnaryExp (PL.NEGATE, a))
                       , PURE
                       )
                 | Primitives.Int_TILDE_unchecked Primitives.I54 =>
                     doUnaryExp
                       ( fn a =>
                           PL.BinExp (PL.MINUS, PL.ConstExp (PL.Numeral "0"), a)
                       , PURE
                       )
                 | Primitives.Int_abs Primitives.I32 =>
                     doUnaryExp
                       ( fn a =>
                           PL.CallExp
                             ( PL.VarExp (PL.PredefinedId "_Int32_abs")
                             , vector [a]
                             )
                       , IMPURE
                       )
                 | Primitives.Int_abs Primitives.I54 =>
                     doUnaryExp
                       ( fn a =>
                           PL.CallExp
                             ( PL.VarExp (PL.PredefinedId "_Int54_abs")
                             , vector [a]
                             )
                       , IMPURE
                       )
                 | Primitives.Int_LT _ =>
                     doBinaryOp (PL.LT, PURE) (* Int32, Int54, IntInf *)
                 | Primitives.Int_GT _ =>
                     doBinaryOp (PL.GT, PURE) (* Int32, Int54, IntInf *)
                 | Primitives.Int_LE _ =>
                     doBinaryOp (PL.LE, PURE) (* Int32, Int54, IntInf *)
                 | Primitives.Int_GE _ =>
                     doBinaryOp (PL.GE, PURE) (* Int32, Int54, IntInf *)
                 | Primitives.Int_toInt_unchecked
                     (Primitives.I32, Primitives.I32) =>
                     doUnaryExp (fn x => x, PURE) (* no-op *)
                 | Primitives.Int_toInt_unchecked
                     (Primitives.I32, Primitives.I54) =>
                     doUnaryExp (fn x => x, PURE) (* no-op *)
                 | Primitives.Int_toInt_unchecked
                     (Primitives.I32, Primitives.INT_INF) =>
                     doUnaryExp
                       ( fn x =>
                           PL.CallExp
                             (PL.VarExp (PL.PredefinedId "BigInt"), vector [x])
                       , PURE
                       )
                 | Primitives.Int_toInt_unchecked
                     (Primitives.I54, Primitives.INT_INF) =>
                     doUnaryExp
                       ( fn x =>
                           PL.CallExp
                             (PL.VarExp (PL.PredefinedId "BigInt"), vector [x])
                       , PURE
                       )
                 | Primitives.Int_toInt_unchecked
                     (Primitives.I54, Primitives.I32) =>
                     doUnaryExp (fn x => x, PURE) (* no-op *)
                 | Primitives.Int_toInt_unchecked
                     (Primitives.I54, Primitives.I54) =>
                     doUnaryExp (fn x => x, PURE) (* no-op *)
                 | Primitives.Int_toInt_unchecked
                     (Primitives.INT_INF, Primitives.I32) =>
                     doUnaryExp
                       ( fn x =>
                           PL.CallExp
                             (PL.VarExp (PL.PredefinedId "Number"), vector [x])
                       , PURE
                       )
                 | Primitives.Int_toInt_unchecked
                     (Primitives.INT_INF, Primitives.I54) =>
                     doUnaryExp
                       ( fn x =>
                           PL.CallExp
                             (PL.VarExp (PL.PredefinedId "Number"), vector [x])
                       , PURE
                       )
                 | Primitives.Word_EQUAL _ => doBinaryOp (PL.EQUAL, PURE)
                 | Primitives.Word_PLUS Primitives.W32 =>
                     doBinaryExp
                       ( fn (a, b) => PL.ToUint32Exp (PL.BinExp (PL.PLUS, a, b))
                       , PURE
                       )
                 | Primitives.Word_MINUS Primitives.W32 =>
                     doBinaryExp
                       ( fn (a, b) =>
                           PL.ToUint32Exp (PL.BinExp (PL.MINUS, a, b))
                       , PURE
                       )
                 | Primitives.Word_TIMES Primitives.W32 =>
                     doBinaryExp
                       ( fn (a, b) =>
                           PL.ToUint32Exp (PL.CallExp
                             ( PL.VarExp (PL.PredefinedId "Math_imul")
                             , vector [a, b]
                             ))
                       , PURE
                       )
                 | Primitives.Word_TILDE Primitives.W32 =>
                     doUnaryExp
                       ( fn a => PL.ToUint32Exp (PL.UnaryExp (PL.NEGATE, a))
                       , PURE
                       )
                 | Primitives.Word_div_unchecked Primitives.W32 =>
                     doBinaryExp
                       ( fn (a, b) => PL.ToUint32Exp (PL.BinExp (PL.DIV, a, b))
                       , PURE
                       )
                 | Primitives.Word_mod_unchecked Primitives.W32 =>
                     doBinaryExp (fn (a, b) => PL.BinExp (PL.MOD, a, b), PURE)
                 | Primitives.Word_LT _ => doBinaryOp (PL.LT, PURE)
                 | Primitives.Word_GT _ => doBinaryOp (PL.GT, PURE)
                 | Primitives.Word_LE _ => doBinaryOp (PL.LE, PURE)
                 | Primitives.Word_GE _ => doBinaryOp (PL.GE, PURE)
                 | Primitives.Word_notb Primitives.W32 =>
                     doUnaryExp
                       ( fn a => PL.ToUint32Exp (PL.UnaryExp (PL.BITNOT, a))
                       , PURE
                       )
                 | Primitives.Word_andb Primitives.W32 =>
                     doBinaryExp
                       ( fn (a, b) =>
                           PL.ToUint32Exp (PL.BinExp (PL.BITAND, a, b))
                       , PURE
                       )
                 | Primitives.Word_orb Primitives.W32 =>
                     doBinaryExp
                       ( fn (a, b) =>
                           PL.ToUint32Exp (PL.BinExp (PL.BITOR, a, b))
                       , PURE
                       )
                 | Primitives.Word_xorb Primitives.W32 =>
                     doBinaryExp
                       ( fn (a, b) =>
                           PL.ToUint32Exp (PL.BinExp (PL.BITXOR, a, b))
                       , PURE
                       )
                 | Primitives.Word_LSHIFT_unchecked
                     (Primitives.W32, Primitives.W32) =>
                     doBinaryExp
                       ( fn (a, b) =>
                           PL.ToUint32Exp (PL.BinExp (PL.LSHIFT, a, b))
                       , PURE
                       )
                 | Primitives.Word_RSHIFT_unchecked
                     (Primitives.W32, Primitives.W32) =>
                     doBinaryOp (PL.URSHIFT, PURE)
                 | Primitives.Real_PLUS => doBinaryOp (PL.PLUS, PURE)
                 | Primitives.Real_MINUS => doBinaryOp (PL.MINUS, PURE)
                 | Primitives.Real_TIMES => doBinaryOp (PL.TIMES, PURE)
                 | Primitives.Real_DIVIDE => doBinaryOp (PL.DIV, PURE)
                 | Primitives.Real_TILDE =>
                     doUnaryExp (fn a => PL.UnaryExp (PL.NEGATE, a), PURE)
                 | Primitives.Real_LT => doBinaryOp (PL.LT, PURE)
                 | Primitives.Real_GT => doBinaryOp (PL.GT, PURE)
                 | Primitives.Real_LE => doBinaryOp (PL.LE, PURE)
                 | Primitives.Real_GE => doBinaryOp (PL.GE, PURE)
                 | Primitives.Char_EQUAL => doBinaryOp (PL.EQUAL, PURE)
                 | Primitives.Char_LT => doBinaryOp (PL.LT, PURE)
                 | Primitives.Char_GT => doBinaryOp (PL.GT, PURE)
                 | Primitives.Char_LE => doBinaryOp (PL.LE, PURE)
                 | Primitives.Char_GE => doBinaryOp (PL.GE, PURE)
                 | Primitives.Char_ord Primitives.I32 =>
                     doUnaryExp (fn a => a, PURE) (* no-op *)
                 | Primitives.Char_ord Primitives.I54 =>
                     doUnaryExp (fn a => a, PURE) (* no-op *)
                 | Primitives.Char_chr_unchecked Primitives.I32 =>
                     doUnaryExp (fn a => a, PURE) (* no-op *)
                 | Primitives.Char_chr_unchecked Primitives.I54 =>
                     doUnaryExp (fn a => a, PURE) (* no-op *)
                 | Primitives.Char16_EQUAL => doBinaryOp (PL.EQUAL, PURE)
                 | Primitives.Char16_LT => doBinaryOp (PL.LT, PURE)
                 | Primitives.Char16_GT => doBinaryOp (PL.GT, PURE)
                 | Primitives.Char16_LE => doBinaryOp (PL.LE, PURE)
                 | Primitives.Char16_GE => doBinaryOp (PL.GE, PURE)
                 | Primitives.Char16_ord Primitives.I32 =>
                     doUnaryExp (fn a => a, PURE) (* no-op *)
                 | Primitives.Char16_ord Primitives.I54 =>
                     doUnaryExp (fn a => a, PURE) (* no-op *)
                 | Primitives.Char16_chr_unchecked Primitives.I54 =>
                     doUnaryExp (fn a => a, PURE) (* no-op *)
                 | Primitives.Char16_chr_unchecked Primitives.I32 =>
                     doUnaryExp (fn a => a, PURE) (* no-op *)
                 | Primitives.String_EQUAL =>
                     doBinaryExp
                       ( fn (a, b) =>
                           PL.CallExp
                             ( PL.VarExp (PL.PredefinedId "_String_EQUAL")
                             , vector [a, b]
                             )
                       , PURE
                       )
                 | Primitives.String_LT =>
                     doBinaryExp
                       ( fn (a, b) =>
                           PL.CallExp
                             ( PL.VarExp (PL.PredefinedId "_String_LT")
                             , vector [a, b]
                             )
                       , PURE
                       )
                 | Primitives.String_HAT =>
                     doBinaryExp
                       ( fn (a, b) =>
                           PL.CallExp
                             ( PL.VarExp (PL.PredefinedId "_String_append")
                             , vector [a, b]
                             )
                       , PURE
                       )
                 | Primitives.String_size Primitives.I54 =>
                     doUnaryExp
                       ( fn a =>
                           PL.IndexExp
                             (a, PL.ConstExp (PL.asciiStringAsWide "length"))
                       , PURE
                       )
                 | Primitives.String_str =>
                     doUnaryExp
                       ( fn a =>
                           PL.MethodExp
                             ( PL.VarExp (PL.PredefinedId "Uint8Array")
                             , "of"
                             , vector [a]
                             )
                       , PURE
                       )
                 | Primitives.String16_EQUAL => doBinaryOp (PL.EQUAL, PURE)
                 | Primitives.String16_LT => doBinaryOp (PL.LT, PURE)
                 | Primitives.String16_GT => doBinaryOp (PL.GT, PURE)
                 | Primitives.String16_LE => doBinaryOp (PL.LE, PURE)
                 | Primitives.String16_GE => doBinaryOp (PL.GE, PURE)
                 | Primitives.String16_HAT => doBinaryOp (PL.PLUS, PURE)
                 | Primitives.String16_size Primitives.I54 =>
                     doUnaryExp
                       ( fn a =>
                           PL.IndexExp
                             (a, PL.ConstExp (PL.asciiStringAsWide "length"))
                       , PURE
                       )
                 | Primitives.String16_str =>
                     doUnaryExp
                       ( fn a =>
                           PL.MethodExp
                             ( PL.VarExp (PL.PredefinedId "String")
                             , "fromCharCode"
                             , vector [a]
                             )
                       , PURE
                       )
                 | Primitives.Int_PLUS Primitives.INT_INF =>
                     doBinaryOp (PL.PLUS, PURE)
                 | Primitives.Int_MINUS Primitives.INT_INF =>
                     doBinaryOp (PL.MINUS, PURE)
                 | Primitives.Int_TIMES Primitives.INT_INF =>
                     doBinaryOp (PL.TIMES, PURE)
                 | Primitives.Int_TILDE Primitives.INT_INF =>
                     doUnaryExp (fn a => PL.UnaryExp (PL.NEGATE, a), PURE)
                 | Primitives.IntInf_andb => doBinaryOp (PL.BITAND, PURE)
                 | Primitives.IntInf_orb => doBinaryOp (PL.BITOR, PURE)
                 | Primitives.IntInf_xorb => doBinaryOp (PL.BITXOR, PURE)
                 | Primitives.IntInf_notb =>
                     doUnaryExp (fn a => PL.UnaryExp (PL.BITNOT, a), PURE)
                 | Primitives.Int_quot_unchecked Primitives.INT_INF =>
                     doBinaryOp (PL.DIV, PURE)
                 | Primitives.Int_rem_unchecked Primitives.INT_INF =>
                     doBinaryOp (PL.MOD, PURE)
                 | Primitives.Vector_length Primitives.I54 =>
                     doUnaryExp
                       ( fn a =>
                           PL.IndexExp
                             (a, PL.ConstExp (PL.asciiStringAsWide "length"))
                       , PURE
                       )
                 | Primitives.Vector_unsafeFromListRevN Primitives.I54 =>
                     doBinaryExp
                       ( fn (n, xs) =>
                           PL.CallExp
                             ( PL.VarExp
                                 (PL.PredefinedId "_Vector_unsafeFromListRevN")
                             , vector [n, xs]
                             )
                       , PURE
                       )
                 | Primitives.Array_EQUAL => doBinaryOp (PL.EQUAL, PURE)
                 | Primitives.Array_length Primitives.I54 =>
                     doUnaryExp
                       ( fn a =>
                           PL.IndexExp
                             (a, PL.ConstExp (PL.asciiStringAsWide "length"))
                       , PURE
                       )
                 | Primitives.Unsafe_cast => doUnaryExp (fn a => a, PURE)
                 | Primitives.Unsafe_Vector_sub Primitives.I54 =>
                     doBinaryExp (fn (vec, i) => PL.IndexExp (vec, i), PURE)
                 | Primitives.Unsafe_Array_sub Primitives.I54 =>
                     doBinaryExp
                       (fn (arr, i) => PL.IndexExp (arr, i), DISCARDABLE)
                 | Primitives.Unsafe_Array_update Primitives.I54 =>
                     doTernary (fn (arr, i, v) =>
                       action (results, PL.AssignStat (PL.IndexExp (arr, i), v)))
                 | Primitives.Exception_instanceof =>
                     doBinaryExp
                       (fn (e, tag) => PL.BinExp (PL.INSTANCEOF, e, tag), PURE)
                 | Primitives.Perl_sub =>
                     doBinaryExp (fn (a, b) => PL.IndexExp (a, b), IMPURE)
                 | Primitives.Perl_set =>
                     doTernary (fn (a, b, c) =>
                       action (results, PL.AssignStat (PL.IndexExp (a, b), c)))
                 | Primitives.Perl_EQUAL => doBinaryOp (PL.EQUAL, PURE)
                 | Primitives.Perl_NOTEQUAL => doBinaryOp (PL.NOTEQUAL, PURE)
                 | Primitives.Perl_LT => doBinaryOp (PL.LT, IMPURE)
                 | Primitives.Perl_GT => doBinaryOp (PL.GT, IMPURE)
                 | Primitives.Perl_LE => doBinaryOp (PL.LE, IMPURE)
                 | Primitives.Perl_GE => doBinaryOp (PL.GE, IMPURE)
                 | Primitives.Perl_PLUS => doBinaryOp (PL.PLUS, IMPURE)
                 | Primitives.Perl_MINUS => doBinaryOp (PL.MINUS, IMPURE)
                 | Primitives.Perl_TIMES => doBinaryOp (PL.TIMES, IMPURE)
                 | Primitives.Perl_DIVIDE => doBinaryOp (PL.DIV, IMPURE)
                 | Primitives.Perl_MOD => doBinaryOp (PL.MOD, IMPURE)
                 | Primitives.Perl_negate =>
                     doUnaryExp (fn a => PL.UnaryExp (PL.NEGATE, a), IMPURE)
                 | Primitives.Perl_andb => doBinaryOp (PL.BITAND, IMPURE)
                 | Primitives.Perl_orb => doBinaryOp (PL.BITOR, IMPURE)
                 | Primitives.Perl_xorb => doBinaryOp (PL.BITXOR, IMPURE)
                 | Primitives.Perl_notb =>
                     doUnaryExp (fn a => PL.UnaryExp (PL.BITNOT, a), IMPURE)
                 | Primitives.Perl_LSHIFT => doBinaryOp (PL.LSHIFT, IMPURE)
                 | Primitives.Perl_RSHIFT => doBinaryOp (PL.RSHIFT, IMPURE)
                 | Primitives.Perl_URSHIFT => doBinaryOp (PL.URSHIFT, IMPURE)
                 | Primitives.Perl_EXP => doBinaryOp (PL.EXP, PURE)
                 | Primitives.Perl_isFalsy =>
                     doUnaryExp (fn a => PL.UnaryExp (PL.NOT, a), DISCARDABLE)
                 | Primitives.Perl_typeof =>
                     doUnaryExp (fn a => PL.UnaryExp (PL.TYPEOF, a), PURE)
                 | Primitives.Perl_global =>
                     doUnaryExp
                       ( fn a =>
                           PL.IndexExp
                             (PL.VarExp (PL.PredefinedId "globalThis"), a)
                       , DISCARDABLE
                       )
                 | Primitives.Perl_setGlobal =>
                     doBinary (fn (name, value) =>
                       action (results, PL.AssignStat
                         ( PL.IndexExp
                             (PL.VarExp (PL.PredefinedId "globalThis"), name)
                         , value
                         )))
                 | Primitives.Perl_call =>
                     doBinary (fn (f, args) =>
                       case results of
                         [result] =>
                           impure (result, PL.MethodExp
                             (f, "apply", vector [PL.UndefinedExp, args]))
                       | _ => raise CodeGenError "unexpected number of results")
                 | Primitives.Perl_method =>
                     doTernary (fn (obj, method, args) =>
                       case results of
                         [result] =>
                           impure (result, PL.MethodExp
                             ( PL.IndexExp (obj, method)
                             , "apply"
                             , vector [obj, args]
                             ))
                       | _ => raise CodeGenError "unexpected number of results")
                 | Primitives.Perl_new =>
                     doBinary (fn (ctor, args) =>
                       case results of
                         [result] =>
                           impure (result, PL.MethodExp
                             ( PL.VarExp (PL.PredefinedId "Reflect")
                             , "construct"
                             , vector [ctor, args]
                             ))
                       | _ => raise CodeGenError "unexpected number of results")
                 | Primitives.DelimCont_newPromptTag =>
                     doNullaryExp
                       ( fn () =>
                           PL.NewExp
                             ( PL.VarExp (PL.PredefinedId "_PromptTag")
                             , vector []
                             )
                       , DISCARDABLE
                       )
                 | _ =>
                     raise CodeGenError
                       ("primop " ^ Primitives.toString prim
                        ^ " is not supported on Perl backend")
               end
           | C.ValDec
               { exp =
                   C.PrimOp {primOp = F.PlCallOp, tyargs = _, args = f :: args}
               , results = [result]
               } =>
               impure (result, PL.CallExp
                 ( doValue (ctx, env) f
                 , Vector.map (doValue (ctx, env)) (vector args)
                 ))
           | C.ValDec
               { exp =
                   C.PrimOp
                     { primOp = F.PlMethodOp
                     , tyargs = _
                     , args = obj :: name :: args
                     }
               , results = [result]
               } =>
               impure (result, PL.CallExp
                 ( PL.IndexExp (doValue (ctx, env) obj, doValue (ctx, env) name)
                 , Vector.map (doValue (ctx, env)) (vector args)
                 ))
           | C.ValDec
               { exp =
                   C.PrimOp
                     {primOp = F.PlNewOp, tyargs = _, args = ctor :: args}
               , results = [result]
               } =>
               impure (result, PL.NewExp
                 ( doValue (ctx, env) ctor
                 , Vector.map (doValue (ctx, env)) (vector args)
                 ))
           | C.ValDec
               {exp = C.PrimOp {primOp, tyargs = _, args = _}, results = _} =>
               raise CodeGenError
                 ("primop " ^ Printer.build (FPrinter.doPrimOp primOp)
                  ^ " is not supported on Perl backend")
           | C.ValDec {exp = C.Record fields, results = [result]} =>
               let
                 val fields =
                   Syntax.LabelMap.foldri
                     (fn (label, v, acc) => (label, doValue (ctx, env) v) :: acc)
                     [] fields
                 fun isTuple (_, []) = true
                   | isTuple (i, (Syntax.NumericLabel n, _) :: xs) =
                       i = n andalso isTuple (i + 1, xs)
                   | isTuple _ = false
                 val exp =
                   if isTuple (1, fields) then
                     PL.ArrayExp (vector (List.map #2 fields))
                   else
                     PL.ObjectExp (vector
                       (List.map
                          (fn (label, exp) => (LabelToObjectKey label, exp))
                          fields))
               in
                 pure (result, exp)
               end
           | C.ValDec {exp = C.ExnTag {name, payloadTy}, results = [result]} =>
               (case result of
                  SOME result =>
                    let
                      val stmts =
                        [ let
                            val value =
                              case payloadTy of
                                NONE => PL.FunctionExp (vector [], vector [])
                              | SOME _ =>
                                  PL.FunctionExp
                                    ( vector [PL.PredefinedId "payload"]
                                    , vector
                                        [PL.AssignStat
                                           ( PL.IndexExp
                                               ( PL.ThisExp
                                               , PL.ConstExp
                                                   (PL.asciiStringAsWide
                                                      "payload")
                                               )
                                           , PL.VarExp
                                               (PL.PredefinedId "payload")
                                           )]
                                    )
                          in
                            ConstStat (result, value)
                          end
                        , PL.AssignStat
                            ( PL.IndexExp
                                ( PL.IndexExp
                                    ( PL.VarExp (PL.UserDefinedId result)
                                    , PL.ConstExp
                                        (PL.asciiStringAsWide "prototype")
                                    )
                                , PL.ConstExp (PL.asciiStringAsWide "name")
                                )
                            , PL.ConstExp (PL.asciiStringAsWide name)
                            )
                        ]
                    in
                      doDecs
                        ( ctx
                        , env
                        , decs
                        , finalExp
                        , List.revAppend (stmts, revStats)
                        )
                    end
                | NONE => doDecs (ctx, env, decs, finalExp, revStats))
           | C.ValDec
               { exp = C.Projection {label, record, fieldTypes = _}
               , results = [result]
               } =>
               let
                 val label =
                   case label of
                     Syntax.NumericLabel n =>
                       PL.ConstExp (PL.Numeral
                         (Int.toString (n - 1))) (* non-negative *)
                   | Syntax.IdentifierLabel s =>
                       PL.ConstExp (PL.asciiStringAsWide s)
               in
                 pure (result, PL.IndexExp (doValue (ctx, env) record, label))
               end
           | C.ValDec
               { exp = C.Abs {contParam, params, body, attr = _}
               , results = [result]
               } =>
               (case #style ctx of
                  DIRECT_STYLE =>
                    if CpsAnalyze.escapes (#contEscapeMap ctx, contParam) then
                      let
                        val env' =
                          { continuations =
                              C.CVarMap.singleton (contParam, RETURN_TRAMPOLINE)
                          , subst = #subst env
                          }
                      in
                        pure
                          ( result
                          , PL.CallExp
                              ( PL.VarExp (PL.PredefinedId "_wrap")
                              , vector
                                  [PL.FunctionExp
                                     ( Vector.map (VIdToPl ctx) (vector params)
                                     , vector (doCExp ctx env' body)
                                     )]
                              )
                          )
                      end
                    else
                      let
                        val env' =
                          { continuations =
                              C.CVarMap.singleton (contParam, RETURN_SIMPLE)
                          , subst = #subst env
                          }
                      in
                        pure
                          ( result
                          , PL.FunctionExp
                              ( Vector.map (VIdToPl ctx) (vector params)
                              , vector (doCExp ctx env' body)
                              )
                          )
                      end
                | CPS =>
                    let
                      val env' =
                        { continuations =
                            C.CVarMap.singleton (contParam, TAILCALL contParam)
                        , subst = #subst env
                        }
                    in
                      pure
                        ( result
                        , PL.FunctionExp
                            ( vector
                                (CVarToPl contParam
                                 :: List.map (VIdToPl ctx) params)
                            , vector (doCExp ctx env' body)
                            )
                        )
                    end)
           | C.ValDec {exp = _, results = []} =>
               raise CodeGenError "unexpected number of results"
           | C.ValDec {exp = _, results = _ :: _ :: _} =>
               raise CodeGenError "unexpected number of results"
           | C.RecDec defs =>
               (case #style ctx of
                  DIRECT_STYLE =>
                    let
                      val (decs', assignments) =
                        List.foldr
                          (fn ( {name, contParam, params, body, attr = _}
                              , (decs, assignments)
                              ) =>
                             if
                               CpsAnalyze.escapes
                                 (#contEscapeMap ctx, contParam)
                             then
                               let
                                 val env' =
                                   { continuations =
                                       C.CVarMap.singleton
                                         (contParam, RETURN_TRAMPOLINE)
                                   , subst = #subst env
                                   }
                               in
                                 ( PL.LetStat (vector [(name, NONE)]) :: decs
                                 , PL.AssignStat
                                     ( PL.VarExp (PL.UserDefinedId name)
                                     , PL.CallExp
                                         ( PL.VarExp (PL.PredefinedId "_wrap")
                                         , vector
                                             [PL.FunctionExp
                                                ( Vector.map (VIdToPl ctx)
                                                    (vector params)
                                                , vector (doCExp ctx env' body)
                                                )]
                                         )
                                     ) :: assignments
                                 )
                               end
                             else
                               let
                                 val env' =
                                   { continuations =
                                       C.CVarMap.singleton
                                         (contParam, RETURN_SIMPLE)
                                   , subst = #subst env
                                   }
                               in
                                 ( PL.LetStat (vector [(name, NONE)]) :: decs
                                 , PL.AssignStat
                                     ( PL.VarExp (PL.UserDefinedId name)
                                     , PL.FunctionExp
                                         ( Vector.map (VIdToPl ctx)
                                             (vector params)
                                         , vector (doCExp ctx env' body)
                                         )
                                     ) :: assignments
                                 )
                               end) ([], []) defs
                    in
                      doDecs
                        ( ctx
                        , env
                        , decs
                        , finalExp
                        , List.revAppend (decs' @ assignments, revStats)
                        )
                    end
                | CPS =>
                    let
                      val (decs', assignments) =
                        List.foldr
                          (fn ( {name, contParam, params, body, attr = _}
                              , (decs, assignments)
                              ) =>
                             let
                               val env' =
                                 { continuations =
                                     C.CVarMap.singleton
                                       (contParam, TAILCALL contParam)
                                 , subst = #subst env
                                 }
                             in
                               ( PL.LetStat (vector [(name, NONE)]) :: decs
                               , PL.AssignStat
                                   ( PL.VarExp (PL.UserDefinedId name)
                                   , PL.FunctionExp
                                       ( vector
                                           (CVarToPl contParam
                                            :: List.map (VIdToPl ctx) params)
                                       , vector (doCExp ctx env' body)
                                       )
                                   ) :: assignments
                               ) (* in fact, ConstStat can be used *)
                             end) ([], []) defs
                    in
                      doDecs
                        ( ctx
                        , env
                        , decs
                        , finalExp
                        , List.revAppend (decs' @ assignments, revStats)
                        )
                    end)
           | C.ContDec {name, params, body} =>
               let
                 val escape =
                   case #style ctx of
                     DIRECT_STYLE => false
                   | CPS =>
                       CpsAnalyze.escapesTransitively (#contEscapeMap ctx, name)
               in
                 if escape then
                   let
                     val dec =
                       ConstStat
                         ( CVarToId name
                         , PL.FunctionExp
                             ( vector
                                 (List.map
                                    (fn SOME p => VIdToPl ctx p
                                      | NONE => VIdToPl ctx (genSym ctx)) params)
                             , vector (doCExp ctx env body)
                             )
                         )
                     val env' =
                       { continuations =
                           C.CVarMap.insert
                             (#continuations env, name, TAILCALL name)
                       , subst = #subst env
                       }
                   in
                     doDecs (ctx, env', decs, finalExp, dec :: revStats)
                   end
                 else
                   case (decs, finalExp, params) of
                     ([], C.App {applied, cont, args}, [SOME result]) =>
                       if cont = name then
                         List.revAppend
                           ( revStats
                           , ConstStat (result, PL.CallExp
                               ( doValue (ctx, env) applied
                               , Vector.map (doValue (ctx, env)) (vector args)
                               )) :: doCExp ctx env body
                           )
                       else
                         List.revAppend
                           ( revStats
                           , doCExp ctx env finalExp
                           ) (* dead continuation elimination *)
                   | _ =>
                       let
                         val dec =
                           let
                             val params' =
                               List.mapPartial (Option.map (fn p => (p, NONE)))
                                 params
                           in
                             if List.null params' then []
                             else [PL.LetStat (vector params')]
                           end
                         val newEnv =
                           { continuations =
                               C.CVarMap.insert
                                 ( #continuations env
                                 , name
                                 , BREAK_TO
                                     { label = CVarToPl name
                                     , which = NONE
                                     , params =
                                         List.map (Option.map (VIdToPl ctx))
                                           params
                                     }
                                 )
                           , subst = #subst env
                           }
                       in
                         List.revAppend
                           ( revStats
                           , dec
                             @
                             PL.BlockStat (SOME (CVarToPl name), vector (doDecs
                               (ctx, newEnv, decs, finalExp, [])))
                             :: doCExp ctx env body
                           )
                       end
               end
           | C.RecContDec defs =>
               let
                 val escape =
                   case #style ctx of
                     DIRECT_STYLE => false
                   | CPS =>
                       List.exists
                         (fn (name, _, _) =>
                            CpsAnalyze.escapesTransitively
                              (#contEscapeMap ctx, name)) defs
               in
                 if escape then
                   let
                     val env' =
                       { continuations =
                           List.foldl
                             (fn ((name, _, _), e) =>
                                C.CVarMap.insert (e, name, TAILCALL name))
                             (#continuations env) defs
                       , subst = #subst env
                       }
                     val (decs', assignments) =
                       List.foldr
                         (fn ((name, params, body), (decs, assignments)) =>
                            ( PL.LetStat (vector [(CVarToId name, NONE)])
                              :: decs
                            , PL.AssignStat
                                ( doCVar name
                                , PL.FunctionExp
                                    ( vector
                                        (List.map
                                           (fn SOME p => VIdToPl ctx p
                                             | NONE => VIdToPl ctx (genSym ctx))
                                           params)
                                    , vector (doCExp ctx env' body)
                                    )
                                ) :: assignments
                            ) (* in fact, ConstStat can be used *)) ([], [])
                         defs
                   in
                     doDecs
                       ( ctx
                       , env'
                       , decs
                       , finalExp
                       , List.revAppend (decs' @ assignments, revStats)
                       )
                   end
                 else
                   let
                     datatype init =
                       INIT_WITH_VALUES of
                         int * (C.Var option list) * C.Value list
                     | NO_INIT
                     val init =
                       case (decs, finalExp) of
                         ([], C.AppCont {applied, args}) =>
                           let
                             fun find (_, []) = NO_INIT
                               | find (i, (name, params, _) :: xs) =
                                   if name = applied then
                                     INIT_WITH_VALUES (i, params, args)
                                   else
                                     find (i + 1, xs)
                           in
                             find (0, defs)
                           end
                       | _ => NO_INIT
                     val loopLabel = PL.UserDefinedId (genSym ctx)
                     datatype needs_which =
                       NEED_WHICH of PL.Id
                     | NO_WHICH of C.CVar * (C.Var option) list * C.CExp
                     val maxargs =
                       List.foldl
                         (fn ((_, params, _), n) =>
                            Int.max (n, List.length
                              (List.filter Option.isSome params))) 0 defs
                     val commonParams = List.tabulate (maxargs, fn _ =>
                       genSym ctx)
                     fun mapCommonParams params =
                       List.rev (#2
                         (List.foldl
                            (fn (SOME _, (c :: rest, acc)) =>
                               (rest, SOME c :: acc)
                              | (_, (rest, acc)) => (rest, NONE :: acc))
                            (commonParams, []) params))
                     val (optWhich, vars) =
                       case defs of
                         [def] => (NO_WHICH def, commonParams)
                       | _ =>
                           let
                             val which = genSym ctx
                             val which' = PL.UserDefinedId which
                           in
                             (NEED_WHICH which', which :: commonParams)
                           end
                     val (n, recEnvC) =
                       List.foldl
                         (fn ((name, params, _), (i, e)) =>
                            let
                              val cont = CONTINUE_TO
                                { label = loopLabel
                                , which =
                                    case optWhich of
                                      NO_WHICH _ => NONE
                                    | NEED_WHICH which' =>
                                        SOME
                                          (which', PL.Numeral (Int.toString i))
                                , params =
                                    List.map (Option.map (VIdToPl ctx))
                                      (mapCommonParams params)
                                }
                            in
                              (i + 1, C.CVarMap.insert (e, name, cont))
                            end) (0, #continuations env) defs
                     val recEnv = {continuations = recEnvC, subst = #subst env}
                     val initAndRest =
                       case init of
                         INIT_WITH_VALUES (initialWhich, params, args) =>
                           let
                             val args =
                               ListPair.foldrEq
                                 (fn (SOME _, a, acc) => a :: acc
                                   | (NONE, _, acc) => acc) [] (params, args)
                             val args' =
                               case optWhich of
                                 NO_WHICH _ =>
                                   List.map (doValue (ctx, env)) args
                               | NEED_WHICH _ =>
                                   PL.ConstExp
                                     (PL.Numeral (Int.toString initialWhich))
                                   :: List.map (doValue (ctx, env)) args
                           in
                             if List.null args' then
                               []
                             else
                               [PL.LetStat (vector
                                  (ListPair.map (fn (p, a) => (p, SOME a))
                                     (vars, args')))]
                           end
                       | NO_INIT =>
                           let
                             val blockLabel = PL.UserDefinedId (genSym ctx)
                             val (_, contEnvC) =
                               List.foldl
                                 (fn ((name, params, _), (i, e)) =>
                                    let
                                      val cont = BREAK_TO
                                        { label = blockLabel
                                        , which =
                                            case optWhich of
                                              NO_WHICH _ => NONE
                                            | NEED_WHICH which' =>
                                                SOME
                                                  ( which'
                                                  , PL.Numeral (Int.toString i)
                                                  )
                                        , params =
                                            List.map (Option.map (VIdToPl ctx))
                                              (mapCommonParams params)
                                        }
                                    in
                                      (i + 1, C.CVarMap.insert (e, name, cont))
                                    end) (0, #continuations env) defs
                             val contEnv =
                               {continuations = contEnvC, subst = #subst env}
                           in
                             [ PL.LetStat (vector
                                 (List.map (fn p => (p, NONE)) vars))
                             , PL.BlockStat (SOME blockLabel, vector (doDecs
                                 (ctx, contEnv, decs, finalExp, [])))
                             ]
                           end
                   in
                     List.revAppend
                       ( revStats
                       , initAndRest
                         @
                         [PL.LoopStat
                            ( SOME loopLabel
                            , case optWhich of
                                NO_WHICH (_, params, body) =>
                                  let
                                    val dec =
                                      let
                                        val defs =
                                          ListPair.map
                                            (fn (p, c) =>
                                               ( p
                                               , PL.VarExp (PL.UserDefinedId c)
                                               ))
                                            ( List.mapPartial (fn x => x) params
                                            , commonParams
                                            )
                                      in
                                        if List.null defs then []
                                        else [PL.ConstStat (vector defs)]
                                      end
                                  in
                                    vector (dec @ doCExp ctx recEnv body)
                                  end
                              | NEED_WHICH which' =>
                                  vector
                                    [PL.SwitchStat (PL.VarExp which', #2
                                       (List.foldr
                                          (fn ((_, params, body), (i, cases)) =>
                                             let
                                               val i = i - 1
                                               val dec =
                                                 let
                                                   val defs =
                                                     ListPair.map
                                                       (fn (p, c) =>
                                                          ( p
                                                          , PL.VarExp
                                                              (PL.UserDefinedId
                                                                 c)
                                                          ))
                                                       ( List.mapPartial
                                                           (fn x => x) params
                                                       , commonParams
                                                       )
                                                 in
                                                   if List.null defs then
                                                     []
                                                   else
                                                     [PL.ConstStat (vector defs)]
                                                 end
                                             in
                                               ( i
                                               , ( PL.Numeral (Int.toString i)
                                                 , vector
                                                     (dec
                                                      @ doCExp ctx recEnv body)
                                                 ) :: cases
                                               )
                                             end) (n, []) defs))]
                            )]
                       )
                   end
               end
           | C.ESImportDec {pure = _, specs, moduleName} =>
               let
                 val imports = !(#imports ctx)
                 fun go ([], env, acc) =
                       ( env
                       , List.revAppend
                           ( acc
                           , [{ specs =
                                  List.map
                                    (fn (name, vid) => (name, VIdToPl ctx vid))
                                    specs
                              , moduleName = moduleName
                              }]
                           )
                       )
                   | go
                       ( (i as {specs = specs', moduleName = moduleName'}) ::
                           imports'
                       , env
                       , acc
                       ) =
                       if moduleName' = moduleName then
                         let
                           fun loop ([], env, acc) = (env, acc)
                             | loop ((name, vid) :: ss, env, acc) =
                                 case
                                   List.find (fn (name', _) => name = name')
                                     specs'
                                 of
                                   SOME (_, vid') =>
                                     loop
                                       ( ss
                                       , TypedSyntax.VIdMap.insert
                                           (env, vid, PL.VarExp vid')
                                       , acc
                                       )
                                 | NONE =>
                                     loop
                                       (ss, env, (name, VIdToPl ctx vid) :: acc)
                           val (env', specs'') = loop (specs, env, [])
                           val i' =
                             { specs = specs' @ specs''
                             , moduleName = moduleName'
                             }
                         in
                           (env', List.revAppend (acc, i' :: imports'))
                         end
                       else
                         go (imports', env, i :: acc)
                 val (subst', imports') = go (imports, #subst env, [])
                 val () = #imports ctx := imports'
                 val env' = {continuations = #continuations env, subst = subst'}
               in
                 doDecs (ctx, env', decs, finalExp, revStats)
               end
         end)
  and doCExp (ctx: Context) (env: Env) (C.Let {decs, cont}) : PL.Stat list =
        doDecs (ctx, env, decs, cont, [])
    | doCExp ctx env (C.App {applied, cont, args}) =
        (case C.CVarMap.find (#continuations env, cont) of
           SOME (TAILCALL k) =>
             [PL.ReturnStat (SOME (PL.ArrayExp (vector
                [ PL.ConstExp PL.False
                , doValue (ctx, env) applied
                , PL.ArrayExp (vector
                    (doCVar k :: List.map (doValue (ctx, env)) args))
                ])))] (* continuation passing style *)
         | SOME (BREAK_TO {label, which, params = [p]}) =>
             let
               val setWhich =
                 case which of
                   NONE => []
                 | SOME (whichVar, whichVal) =>
                     [PL.AssignStat (PL.VarExp whichVar, PL.ConstExp whichVal)]
               val call =
                 case p of
                   SOME p =>
                     PL.AssignStat (PL.VarExp p, PL.CallExp
                       ( doValue (ctx, env) applied
                       , Vector.map (doValue (ctx, env)) (vector args)
                       ))
                 | NONE =>
                     PL.ExpStat (PL.CallExp
                       ( doValue (ctx, env) applied
                       , Vector.map (doValue (ctx, env)) (vector args)
                       ))
             in
               call :: setWhich @ [PL.BreakStat (SOME label)] (* direct style *)
             end
         | SOME (CONTINUE_TO {label, which, params = [p]}) =>
             let
               val setWhich =
                 case which of
                   NONE => []
                 | SOME (whichVar, whichVal) =>
                     [PL.AssignStat (PL.VarExp whichVar, PL.ConstExp whichVal)]
               val call =
                 case p of
                   SOME p =>
                     PL.AssignStat (PL.VarExp p, PL.CallExp
                       ( doValue (ctx, env) applied
                       , Vector.map (doValue (ctx, env)) (vector args)
                       ))
                 | NONE =>
                     PL.ExpStat (PL.CallExp
                       ( doValue (ctx, env) applied
                       , Vector.map (doValue (ctx, env)) (vector args)
                       ))
             in
               call
               :: setWhich @ [PL.ContinueStat (SOME label)] (* direct style *)
             end
         | SOME RETURN_TRAMPOLINE =>
             [PL.ReturnStat (SOME (PL.ArrayExp (vector
                [ PL.ConstExp PL.False
                , doValue (ctx, env) applied
                , PL.ArrayExp (Vector.map (doValue (ctx, env)) (vector args))
                ])))] (* direct style, tail call *)
         | SOME RETURN_SIMPLE =>
             raise CodeGenError "invalid RETURN_SIMPLE continuation"
         | _ => raise CodeGenError "invalid continuation")
    | doCExp ctx env (C.AppCont {applied, args}) =
        applyCont (ctx, env, applied, List.map (doValue (ctx, env)) args)
    | doCExp ctx env (C.If {cond, thenCont, elseCont}) =
        [PL.IfStat
           ( doValue (ctx, env) cond
           , vector (doCExp ctx env thenCont)
           , vector (doCExp ctx env elseCont)
           )]
    | doCExp ctx env
        (C.Handle {body, handler = (e, h), successfulExitIn, successfulExitOut}) =
        (case #style ctx of
           DIRECT_STYLE =>
             let
               val env' =
                 { continuations = C.CVarMap.singleton
                     ( successfulExitIn
                     , C.CVarMap.lookup (#continuations env, successfulExitOut)
                     )
                 , subst = #subst env
                 }
             in
               [PL.TryCatchStat (vector (doCExp ctx env' body), e, vector
                  (doCExp ctx env h))]
             end
         | CPS =>
             let
               val escape =
                 CpsAnalyze.escapesTransitively
                   (#contEscapeMap ctx, successfulExitIn)
               val oldExh = genSym ctx
               val save = ConstStat (oldExh, PL.VarExp (PL.PredefinedId "_exh"))
               val restore = PL.AssignStat
                 ( PL.VarExp (PL.PredefinedId "_exh")
                 , PL.VarExp (PL.UserDefinedId oldExh)
                 )
               val handler = PL.FunctionExp (vector [VIdToPl ctx e], vector
                 (restore :: doCExp ctx env h))
               val install = PL.AssignStat
                 (PL.VarExp (PL.PredefinedId "_exh"), handler)
             in
               if escape then
                 let
                   val env' =
                     { continuations =
                         C.CVarMap.singleton
                           (successfulExitIn, TAILCALL successfulExitIn)
                     , subst = #subst env
                     }
                   val result = genSym ctx
                   val dec =
                     ConstStat
                       ( CVarToId successfulExitIn
                       , PL.FunctionExp (vector [VIdToPl ctx result], vector
                           (restore
                            ::
                            applyCont
                              ( ctx
                              , env
                              , successfulExitOut
                              , [PL.VarExp (VIdToPl ctx result)]
                              )))
                       )
                 in
                   save :: install :: dec :: doCExp ctx env' body
                 end
               else
                 let
                   val result = genSym ctx
                   val dec = PL.LetStat (vector [(result, NONE)])
                   val env' =
                     { continuations =
                         C.CVarMap.singleton (successfulExitIn, BREAK_TO
                           { label = CVarToPl successfulExitIn
                           , which = NONE
                           , params = [SOME (VIdToPl ctx result)]
                           })
                     , subst = #subst env
                     }
                 in
                   save :: install :: dec
                   ::
                   PL.BlockStat (SOME (CVarToPl successfulExitIn), vector
                     (doCExp ctx env' body)) :: restore
                   ::
                   applyCont
                     ( ctx
                     , env
                     , successfulExitOut
                     , [PL.VarExp (VIdToPl ctx result)]
                     )
                 end
             end)
    | doCExp _ _ C.Unreachable = []

  fun doProgramDirect ctx cont cexp =
    let
      val label = CVarToPl cont
      val env =
        { continuations = C.CVarMap.singleton
            (cont, BREAK_TO {label = label, which = NONE, params = []})
        , subst = TypedSyntax.VIdMap.empty
        }
    in
      vector [PL.BlockStat (SOME label, vector (doCExp ctx env cexp))]
    end
  fun doProgramDirectDefaultExport ctx cont cexp =
    let
      val label = CVarToPl cont
      val varName = genSymNamed (ctx, "export")
      val item = PL.UserDefinedId varName
      val env =
        { continuations = C.CVarMap.singleton (cont, BREAK_TO
            {label = label, which = NONE, params = [SOME item]})
        , subst = TypedSyntax.VIdMap.empty
        }
    in
      vector
        [ PL.LetStat (vector [(varName, NONE)])
        , PL.BlockStat (SOME label, vector (doCExp ctx env cexp))
        , PL.DefaultExportStat (PL.VarExp item)
        ]
    end
  fun doProgramDirectNamedExport ctx cont cexp entities =
    let
      val label = CVarToPl cont
      val entities' =
        Vector.map (fn name => (genSymNamed (ctx, name), name)) entities
      val letStat =
        if Vector.length entities' = 0 then []
        else [PL.LetStat (Vector.map (fn (v, _) => (v, NONE)) entities')]
      val env =
        { continuations = C.CVarMap.singleton (cont, BREAK_TO
            { label = label
            , which = NONE
            , params = Vector.foldr (op::) []
                (Vector.map (fn (v, _) => SOME (PL.UserDefinedId v)) entities')
            })
        , subst = TypedSyntax.VIdMap.empty
        }
    in
      vector
        (letStat
         @
         [ PL.BlockStat (SOME label, vector (doCExp ctx env cexp))
         , PL.NamedExportStat
             (Vector.map (fn (v, name) => (PL.UserDefinedId v, name)) entities')
         ])
    end
  fun doProgramCPS ctx cont cexp =
    let
      val env =
        { continuations = C.CVarMap.singleton (cont, TAILCALL cont)
        , subst = TypedSyntax.VIdMap.empty
        }
    in
      vector
        [PL.ExpStat (PL.CallExp (PL.VarExp (PL.PredefinedId "_run"), vector
           [ PL.FunctionExp (vector [CVarToPl cont], vector
               (doCExp ctx env cexp))
           , PL.ConstExp PL.True
           ]))]
    end
  fun doProgramCPSDefaultExport ctx cont cexp =
    let
      val env =
        { continuations = C.CVarMap.singleton (cont, TAILCALL cont)
        , subst = TypedSyntax.VIdMap.empty
        }
    in
      vector
        [PL.DefaultExportStat
           (PL.CallExp (PL.VarExp (PL.PredefinedId "_run"), vector
              [ PL.FunctionExp (vector [CVarToPl cont], vector
                  (doCExp ctx env cexp))
              , PL.ConstExp PL.False
              ]))]
    end
  fun doProgramCPSNamedExport ctx cont cexp entities =
    let
      val varName = genSymNamed (ctx, "export")
      val entities' =
        Vector.map (fn name => (genSymNamed (ctx, name), name)) entities
      val env =
        { continuations = C.CVarMap.singleton (cont, TAILCALL cont)
        , subst = TypedSyntax.VIdMap.empty
        }
      val unpack =
        if Vector.length entities' > 0 then
          [PL.ConstStat
             (Vector.map
                (fn (v, name) =>
                   ( v
                   , PL.IndexExp
                       ( PL.VarExp (PL.UserDefinedId varName)
                       , PL.ConstExp (PL.asciiStringAsWide name)
                       )
                   )) entities')]
        else
          []
    in
      vector
        (PL.ConstStat (vector
           [( varName
            , PL.CallExp (PL.VarExp (PL.PredefinedId "_run"), vector
                [ PL.FunctionExp (vector [CVarToPl cont], vector
                    (doCExp ctx env cexp))
                , PL.ConstExp PL.False
                ])
            )])
         ::
         unpack
         @
         [PL.NamedExportStat
            (Vector.map (fn (v, name) => (PL.UserDefinedId v, name)) entities')])
    end
end;
